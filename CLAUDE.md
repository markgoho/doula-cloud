...
One family is checked the other way round. British English doubles a final `l` before `-ed`/`-ing` whatever the stress, American English only on a stressed final syllable or a base word already ending in `ll`, so the gate treats every `-alled`/`-elled`/`-alling`/`-elling` word as British unless it is named in the spec's `KEEPS_ITS_DOUBLE_L` allowlist. An American word the allowlist has not met yet — "preinstalling", "rappelled" — is a missing allowlist entry, not an exception: add it there beside the words it belongs with, rather than reaching for `spelling:ignore`, which would silence every other rule on that line too.

`app/src/lib/spelling.usage.spec.ts` enforces this over `app/src`, `app/e2e`, and `api/`'s Go source and fails the build on a new British spelling; it reads the `spelling:ignore` marker as the deliberate override. A few things are outside the sweep because none of them can carry a marker: `bun.lock`, which is generated; `docs/design/doula-cloud.pen`, which is generated by the pen.dev desktop app and never hand-edited (it is plain UTF-8 JSON, not encrypted — see ADR-0019 and `docs/design/doula-cloud.export.md` for a plain-text read-back of it); everything under `docs/research/transcripts/`, which is verbatim auto-generated captions; and `api/db/migrations/`, whose already-applied files goose never re-runs once a version is recorded as applied — so an edited SQL statement would silently disagree with what already ran against the database for anyone who has already run it, while an edited comment is harmless, since goose never reads or stores one.
