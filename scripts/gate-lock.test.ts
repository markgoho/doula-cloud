...
const reclaimReason = (verdict: LockVerdict): string =>
  verdict.action === 'reclaim' ? verdict.reason : '';
...
describe('inspectLock', () => {
  test('waits on a fresh lock whose owner is alive', () => {
    expect(inspectLock(1000, owner(4242), alwaysAlive)).toEqual({
      action: 'wait',
    });
  });

  test('reclaims as soon as the owner process is gone, however fresh the lock', () => {
    expect(reclaimReason(inspectLock(0, owner(4242), neverAlive))).toContain(
      '4242'
    );
  });
  ...
  test('reclaims a lock held past the staleness gate even while its owner lives', () => {
    expect(
      reclaimReason(inspectLock(STALE_LOCK_MS + 1000, owner(4242), alwaysAlive))
    ).toContain('staleness gate');
  });

  test('does not reclaim a lock held exactly at the staleness gate', () => {
    expect(inspectLock(STALE_LOCK_MS, owner(4242), alwaysAlive)).toEqual({
      action: 'wait',
    });
  });
  ...
  test('falls back to the age gate for a lock with no readable owner', () => {
    expect(inspectLock(STALE_LOCK_MS + 1, null, alwaysAlive).action).toBe(
      'reclaim'
    );
  });

  test('accepts a custom staleness threshold', () => {
    expect(inspectLock(2000, owner(4242), alwaysAlive, 1000).action).toBe(
      'reclaim'
    );
  });
});
...
describe('the wrapper', () => {
  ...
  test('leaves a successor lock alone when its own was reclaimed mid-run', async () => {
    ...
    expect(JSON.parse(fs.readFileSync(ownerFile, 'utf8')).label).toBe(
      'successor'
    );
  }, 30000);
  ...
  test('releases the lock when it is interrupted before the command is even spawned', async () => {
    const child = spawn('bun', [SCRIPT, '--', 'bun', marker, journal, '0'], {
      ...
      env: {
        ...
        GATE_LOCK_PRE_SPAWN_DELAY_MS: '1000',
      },
    });
    while (!fs.existsSync(path.join(lockDir, 'owner.json')))
      await new Promise((resolve) => setTimeout(resolve, 5));
