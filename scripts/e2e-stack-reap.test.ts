...
function composeContainer(project: string, service: string, createdAtMs: number): ReapCandidate {
	return {
...
function stack(project: string, offset: number, newestCreatedAtMs: number): StackProject {
	return { project, offset, newestCreatedAtMs, containerIds: [`${project}_db`] };
}
...
describe("groupStackProjects", () => {
	...
	test("keeps each worktree's stack separate", () => {
		const containers = [composeContainer("doula-cloud-e2e-1", "db", 1000), composeContainer("doula-cloud-e2e-2", "db", 2000)];
		expect(groupStackProjects(containers).map(project => project.offset)).toEqual([1, 2]);
	});

	test("ignores the main checkout's unsuffixed stack", () => {
		expect(groupStackProjects([composeContainer("doula-cloud-e2e", "db", 1000)])).toEqual([]);
	});

	test("ignores another project's compose containers", () => {
		expect(groupStackProjects([composeContainer("some-other-project-4", "db", 1000)])).toEqual([]);
	});

	test("ignores a name that only starts like ours", () => {
		expect(groupStackProjects([composeContainer("doula-cloud-e2e-attachments", "db", 1000)])).toEqual([]);
	});
...
describe("pickReapProjects", () => {
	...
	test("reaps a stack older than the threshold whose offset no live worktree claims", () => {
		const orphan = stack("doula-cloud-e2e-4", 4, now - REAP_THRESHOLD_MS - 1000);
	...
	test("only reaps the stacks that qualify out of a mixed list", () => {
		const orphan = stack("doula-cloud-e2e-1", 1, now - REAP_THRESHOLD_MS - 1000);
		const held = stack("doula-cloud-e2e-2", 2, now - REAP_THRESHOLD_MS - 1000);
		const fresh = stack("doula-cloud-e2e-3", 3, now - 1000);
		expect(pickReapProjects([orphan, held, fresh], new Set([2]), now)).toEqual([orphan]);
	});
});
...
describe("liveWorktreeOffsets", () => {
	...
	function worktreeWithOffset(root: string, name: string, offset: string, ageMs: number): string {
	...
	test("claims nothing for an unreadable offset file", () => {
		const root = temporaryWorktreesRoot();
		const worktree = worktreeWithOffset(root, "agent-garbled", "not-a-number", 0);
	...
	test("returns nothing when the worktrees directory does not exist", () => {
		expect(liveWorktreeOffsets(path.join(temporaryWorktreesRoot(), "absent"), now)).toEqual(new Set());
	});
