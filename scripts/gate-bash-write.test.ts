...
describe('gate-bash-write', () => {
  ...
  test('allows redirection outside the repo', async () => {
    const { exitCode } = await invoke(
      'echo hi > /tmp/gate-bash-write-scratch.txt'
    );
  ...
  test('allows a redirect target held in a shell variable ($VAR form)', async () => {
    const { exitCode } = await invoke('printf hi >> $S/c298.md', {
      cwd: SOURCE_ROOT,
    });
    expect(exitCode).toBe(0);
  });
  ...
  test('allows a redirect target held in a command substitution', async () => {
    const { exitCode } = await invoke(
      'printf hi >> $(dirname /tmp/x)/c298.md',
      { cwd: SOURCE_ROOT }
    );
    expect(exitCode).toBe(0);
  });
  ...
  describe("#677: prose containing a literal '>' is not read as redirection", () => {
    test('an HTML tag in a commit message (#451 case 1)', async () => {
      const { exitCode } = await invoke(
        'git commit -m "Add <Badge> next to price"',
        { cwd: SOURCE_ROOT }
      );
      expect(exitCode).toBe(0);
    });
    ...
    test('an ASCII arrow inside a double-quoted echo argument', async () => {
      const { exitCode } = await invoke('echo "1085 -> Ready for human"', {
        cwd: SOURCE_ROOT,
      });
      expect(exitCode).toBe(0);
    });
  ...
  describe('#677: gh issue/pr write-invocations are exempt regardless of body content', () => {
    test('gh issue create with a prose-only body', async () => {
      const { exitCode } = await invoke(
        'gh issue create --title "x" --body "just prose, no redirection"',
        {
          cwd: SOURCE_ROOT,
        }
      );
    ...
    test('gh issue edit with an arrow in the body', async () => {
      const { exitCode } = await invoke(
        'gh issue edit 677 --body "1085 -> Ready for human"',
        {
          cwd: SOURCE_ROOT,
        }
      );
    ...
    test('gh issue comment with an arrow in the body', async () => {
      const { exitCode } = await invoke(
        'gh issue comment 677 --body "1085 -> Ready for human"',
        {
          cwd: SOURCE_ROOT,
        }
      );
    ...
    test('gh pr create with an HTML tag in the body', async () => {
      const { exitCode } = await invoke(
        'gh pr create --title "x" --body "Add <Badge> next to price"',
        {
          cwd: SOURCE_ROOT,
        }
      );
    ...
    test('gh pr edit with an HTML tag in the body', async () => {
      const { exitCode } = await invoke(
        'gh pr edit 1 --body "Add <Badge> next to price"',
        {
          cwd: SOURCE_ROOT,
        }
      );
    ...
    test('still blocks a real trailing redirect after a gh issue create call', async () => {
      const { exitCode } = await invoke(
        `gh issue create --title "x" --body "y" > ${TARGET}`,
        {
          cwd: SOURCE_ROOT,
        }
      );
  ...
  describe('#680: a leading `cd` sets the directory a relative write target resolves against', () => {
    ...
    test('allows `sed -i` on a relative path after a `cd` into a worktree', async () => {
      const { exitCode } = await invoke(
        `cd ${WORKTREE} && sed -i '' 's/x/y/' CLAUDE.md`,
        {
          cwd: SOURCE_ROOT,
        }
      );
    ...
    test('allows a relative path after a chain of `cd`s that lands in a worktree', async () => {
      const { exitCode } = await invoke(
        `cd ${POOL} && cd some-branch && echo hi > CLAUDE.md`,
        {
          cwd: SOURCE_ROOT,
        }
      );
    ...
    test('still blocks a relative path after a `cd` into the main checkout', async () => {
      const { exitCode } = await invoke(
        `cd ${SOURCE_ROOT} && sed -i '' 's/x/y/' CLAUDE.md`,
        {
          cwd: SOURCE_ROOT,
        }
      );
    ...
    test('still blocks a relative path with no `cd` at all', async () => {
      const { exitCode } = await invoke("sed -i '' 's/x/y/' CLAUDE.md", {
        cwd: SOURCE_ROOT,
      });
      expect(exitCode).toBe(2);
    });
    ...
    test('still blocks an absolute tracked target written after a `cd` into a worktree', async () => {
      const { exitCode } = await invoke(
        `cd ${WORKTREE} && sed -i '' 's/x/y/' ${TARGET}`,
        {
          cwd: SOURCE_ROOT,
        }
      );
    ...
    test('still blocks when the `cd` has no argument at all', async () => {
      const { exitCode } = await invoke("cd && sed -i '' 's/x/y/' CLAUDE.md", {
        cwd: SOURCE_ROOT,
      });
      expect(exitCode).toBe(2);
    });
    ...
    test('still blocks when the `cd` argument is a home-relative path', async () => {
      const { exitCode } = await invoke(
        "cd ~/Github && sed -i '' 's/x/y/' CLAUDE.md",
        { cwd: SOURCE_ROOT }
      );
      expect(exitCode).toBe(2);
    });
    ...
    describe('a `cd` does not survive any separator but `&&`', () => {
      ...
      test('`|` puts the `cd` in a pipeline subshell whose directory dies with it', async () => {
        const { exitCode } = await invoke(
          `cd ${WORKTREE} | true && sed -i '' 's/x/y/' CLAUDE.md`,
          {
            cwd: SOURCE_ROOT,
          }
        );
