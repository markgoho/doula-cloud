...
describe('renderPage', () => {
  ...
  test('pins the URL to the slug and nothing else', () => {
    ...
    const renamed = frontMatter(
      renderPage({ ...page, name: 'Genesee Birth Collective' })
    );
  ...
  test("puts a Practice's own words in front matter, never in the body", () => {
    ...
    expect(frontMatter(rendered).cancellationPolicy).toBe(
      hostile.cancellationPolicy
    );
  });
...
describe('writePages', () => {
  test('writes one leaf bundle per page and removes what is no longer published', async () => {
    ...
    const written = await readFile(
      path.join(root, 'rochester-doulas', 'index.md'),
      'utf8'
    );
