...
var roleFreeWriteRoutes = map[string]string{
	...
	"PUT /api/practices/{practiceId}/engagements/{engagementId}/kind":          "ADR-0015's role table (engagement.refuseFactWrite): an Owner, an Admin or an employee Doula, but not a contractor Doula who also holds the doula role, and narrowed again once the birth outcome is recorded (refuseUpgradeAfterBirth refuses postpartum -> birth outright) -- a role-and-employment-type rule, further state-dependent, that a flat mount role list cannot express",
