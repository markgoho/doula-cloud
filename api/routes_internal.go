...
func registerInternalRoutes(g *staffauth.GatedRouter, d Deps) {
	...
	outbox.Register(g, d.DB, d.WorkerSecret, outboxRegistrations(d))
	...
	g.Write("POST /api/internal/site/verify-pages", sitebuild.VerifyHandler(d.DB, d.PageVerifier, d.WorkerSecret))
	...
	// view of anything.
	g.Write("POST /api/internal/billing/refunds", billing.RefundHandler(d.DB, d.StripeClient, d.WorkerSecret))
	g.OpenGet("/api/internal/billing/dormant-practices",
		"X-Internal-Secret, not a session -- the operator's escheatment mailing list (#420), the same position every /api/internal worker endpoint holds",
		billing.DormantPracticesHandler(d.DB, d.WorkerSecret))
	...
	g.Write("POST /api/internal/billing/founding-grants", billing.FoundingGrantHandler(d.DB, d.WorkerSecret))
	...
	g.Write("POST /api/internal/staffauth/mfa-recovery/support-clear", staffauth.SupportClearHandler(d.AccountManager, d.DB, d.WorkerSecret))
}
