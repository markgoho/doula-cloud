...
func TestRoutes_EveryDeclaredGETHasRoleDeclaration(t *testing.T) {
	_, registry := routes(authntest.Verifier{}, nil, objectstore.NewMemoryStore(), push.NewFakePusher(), billing.NewFakeStripeClient(), "whsec_test", payments.NewFakeClient(), "whsec_connect_test", "whsec_account_test", testWorker, testWorkerSecret, testLowCreditWorker, testPayoutOutboxWorker, []string{testExpectedOrigin})
	if len(registry) == 0 {
