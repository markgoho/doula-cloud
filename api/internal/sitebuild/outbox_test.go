...
func buildHandler(db *testdb.DB, d sitebuild.Dispatcher) http.Handler {
	return outbox.ProcessHandler(db.App, sitebuild.Worker{Dispatcher: d, Now: fixedNow}, internalauth.FromSecret(workerSecret), "")
}
...
func TestProcessOutbox_EmptyConfiguredSecretRefusesEveryone(t *testing.T) {
	db := testdb.New(t)
	h := outbox.ProcessHandler(db.App, sitebuild.Worker{Dispatcher: &fakeDispatcher{}, Now: fixedNow}, internalauth.FromSecret(""), "")
