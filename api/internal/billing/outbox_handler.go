...
// ADR-0010's accepted per-Notification-type cost, not a shared one,
// since the two workers process unrelated tables. There is no Staff or
...
func ProcessOutboxHandler(db *sql.DB, worker Worker, secret string) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// Licenses the staff/practice_memberships SELECT policies
		// 00033_low_credit_outbox.sql added for this worker.
