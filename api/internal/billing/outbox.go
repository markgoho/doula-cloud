...
// the Practice currently has rather than one fixed address a Compose
// function could return. outbox.Mailer is still embedded, so no field
...
func (w Worker) inner() outbox.Worker {
	return w.Worker("low_credit_outbox")
}
