...
// no more specific detail -- mirrors staffauth.MsgInternalError, kept
// local since this handler has no session or Staff population of its
// own to import that constant from.
const msgInternalError = "internal error"

// ProcessOutboxHandler is the internal endpoint Cloud Scheduler invokes
// on a fixed cadence to run Worker.ProcessPending, mirroring
// billing.ProcessOutboxHandler/payments.ProcessOutboxHandler's shape --
// a separate endpoint per ADR-0010's accepted per-Notification-type
// cost, since this worker processes its own outbox table. There is no
// Staff or Client session on this path, so it authenticates the caller
// against secret instead. secret must be non-empty: an empty configured
// secret refuses every request rather than accepting an
// unauthenticated one.
func ProcessOutboxHandler(db *sql.DB, worker Worker, secret string) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// Licenses the staff SELECT policy 00033_low_credit_outbox.sql
		// added for billing.Worker's ownerEmails, reused here for
		// staffEmail.
