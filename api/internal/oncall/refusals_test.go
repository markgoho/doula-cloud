...
func TestWrites_RefuseWhatHasNoWindowOrNoEngagement(t *testing.T) {
	...
	decode[map[string]any](t, authedBody(t, session, http.MethodPut,
		ruleURL(srv.URL, f.practiceID, unknown), oncall.RuleRequest{StartRule: &granted}), http.StatusNotFound)
}
...
func TestEngagementRule_AWeekChangeUnderTheSameRuleIsRecorded(t *testing.T) {
	...
	for _, week := range []int{38, 39, 39} {
		decode[oncall.RuleRequest](t, authedBody(t, session, http.MethodPut,
			ruleURL(srv.URL, f.practiceID, f.engagementID)+"?w="+string(rune('0'+week%10)),
...
func TestEngagementRule_RefusesEveryMalformedOverride(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"a week with no rule to apply it to":  {oncall.RuleRequest{StartWeek: &week}, "startWeek"},
		"a week outside the bounds":           {oncall.RuleRequest{StartRule: &gestational, StartWeek: &tooEarly}, "startWeek"},
		"a gestational rule with no week":     {oncall.RuleRequest{StartRule: &gestational}, "startWeek"},
		"a rule name the product does not do": {oncall.RuleRequest{StartRule: &unknown}, "startRule"},
	}
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			body := decode[map[string]any](t, authedBody(t, session, http.MethodPut,
				ruleURL(srv.URL, f.practiceID, f.engagementID), tt.req), http.StatusBadRequest)
...
func TestUpdateGap_RefusesTheSameThingsACreateDoes(t *testing.T) {
	...
	noEnd.EndsAt = nil
	body := decode[map[string]any](t, authedBody(t, session, http.MethodPut, f.gapURL(srv.URL), noEnd), http.StatusBadRequest)
	...
	decode[map[string]any](t, authedBody(t, doulaSession, http.MethodPut, f.gapURL(doulaSrv.URL), gapBody(f.backupID, nil)), http.StatusForbidden)
	decode[oncall.Gap](t, authedBody(t, doulaSession, http.MethodPut, f.gapURL(doulaSrv.URL)+"?own", gapBody(f.doulaID, &f.backupID)), http.StatusOK)
}
...
func TestCreateGap_RefusesALongReasonAndTakesABlankCover(t *testing.T) {
	...
	body.Reason = &long
	decode[map[string]any](t, authedBody(t, session, http.MethodPost, gapsURL(srv.URL, f.practiceID, f.engagementID), body), http.StatusBadRequest)

	blank := ""
	body = gapBody(f.doulaID, &blank)
	body.Reason = nil
	gap := decode[oncall.Gap](t, authedBody(t, session, http.MethodPost, gapsURL(srv.URL, f.practiceID, f.engagementID)+"?blank", body), http.StatusCreated)
...
func TestRoster_ADepartedCoveringColleagueIsNamedAsOne(t *testing.T) {
	...
	backupID := testdb.SeedNamedStaffAtPractice(t, db, f.practiceID, "roster-departed-backup", "Bo Backup", []string{doulaRole}, employeeType)
...
func TestRoster_SortsAndSkipsWhatItShould(t *testing.T) {
	...
	got := f.roster(t, db, "2026-10-09", "2026-10-31")
