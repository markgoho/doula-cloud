...
func TestNarrowing_APrimaryAndABackupEachCarryTheirOwnDays(t *testing.T) {
	...
	backupID := testdb.SeedNamedStaffAtPractice(t, db, f.practiceID, "narrow-pair-backup", "Bo Backup", []string{doulaRole}, employeeType)
	testdb.SeedGrantedAttachment(t, db, f.engagementID, backupID)
	srv, session := newServer(t, db, f.ownerUID)
	defer srv.Close()

	to, from := "2026-10-22", "2026-10-23"
	decode[oncall.NarrowingRequest](t, authedBody(t, session, http.MethodPut, narrowURL(srv.URL, f.practiceID, f.engagementID, f.doulaID), oncall.NarrowingRequest{To: &to}), http.StatusOK)
	decode[oncall.NarrowingRequest](t, authedBody(t, session, http.MethodPut, narrowURL(srv.URL, f.practiceID, f.engagementID, backupID), oncall.NarrowingRequest{From: &from}), http.StatusOK)
	...
	if days[f.doulaID] != [2]string{"2026-10-09", "2026-10-22"} || days[backupID] != [2]string{"2026-10-23", "2026-11-13"} {
	...
	decode[oncall.NarrowingRequest](t, authedBody(t, session, http.MethodPut, narrowURL(srv.URL, f.practiceID, f.engagementID, f.doulaID)+"?clear", oncall.NarrowingRequest{}), http.StatusOK)
	panel = decode[oncall.EngagementOnCall](t, authedGet(t, session, panelURL(srv.URL, f.practiceID, f.engagementID)), http.StatusOK)
	for _, d := range panel.Doulas {
		if d.StaffID == f.doulaID && (d.From != nil || *d.OnCallTo != "2026-11-13") {
...
func TestNarrowing_RefusesWhatItShould(t *testing.T) {
	...
	from, to, junk := "2026-10-25", "2026-10-20", "next week"
	body := decode[map[string]any](t, authedBody(t, session, http.MethodPut, narrowURL(srv.URL, f.practiceID, f.engagementID, f.doulaID), oncall.NarrowingRequest{From: &from, To: &to}), http.StatusBadRequest)
	...
	body = decode[map[string]any](t, authedBody(t, session, http.MethodPut, narrowURL(srv.URL, f.practiceID, f.engagementID, f.doulaID)+"?junk", oncall.NarrowingRequest{From: &junk}), http.StatusBadRequest)
	...
	decode[map[string]any](t, authedBody(t, session, http.MethodPut, narrowURL(srv.URL, f.practiceID, f.engagementID, strangerID), oncall.NarrowingRequest{}), http.StatusNotFound)

	doulaSrv, doulaSession := newServer(t, db, "narrow-refuse-doula")
	defer doulaSrv.Close()
	decode[map[string]any](t, authedBody(t, doulaSession, http.MethodPut, narrowURL(doulaSrv.URL, f.practiceID, f.engagementID, f.doulaID), oncall.NarrowingRequest{}), http.StatusForbidden)
}
...
func TestPanel_StatesWhyThereIsNoWindow(t *testing.T) {
	...
	if panel := decode[oncall.EngagementOnCall](t, authedGet(t, session, panelURL(srv.URL, f.practiceID, f.engagementID)), http.StatusOK); panel.NoWindowReason != oncall.NoWindowNoDueDate || panel.Window != nil {
	...
	if panel := decode[oncall.EngagementOnCall](t, authedGet(t, session, panelURL(srv.URL, f.practiceID, f.engagementID)), http.StatusOK); panel.NoWindowReason != oncall.NoWindowNobodyAttached {
