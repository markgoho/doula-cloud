...
func TestSettings_AnOwnerReadsAndStatesTheRule(t *testing.T) {
	...
	got := getJSON[oncall.Settings](t, session, settingsURL(srv.URL, f.practiceID), http.StatusOK)
	...
	doJSON[oncall.Settings](t, session, http.MethodPut, settingsURL(srv.URL, f.practiceID), want, http.StatusOK)
	doJSON[oncall.Settings](t, session, http.MethodPut, settingsURL(srv.URL, f.practiceID)+"?again", want, http.StatusOK)
	if got := getJSON[oncall.Settings](t, session, settingsURL(srv.URL, f.practiceID), http.StatusOK); got != want {
...
func TestSettings_RefusesOutOfRangeValuesAndADoula(t *testing.T) {
	...
	body := doJSON[map[string]any](t, session, http.MethodPut, settingsURL(srv.URL, f.practiceID),
		oncall.Settings{StartRule: "whenever", StartWeek: 12, GraceDays: 60}, http.StatusBadRequest)
	details, _ := body["details"].(map[string]any)
	for _, field := range []string{"startRule", fieldWeek, "graceDays"} {
...
func TestEngagementRule_OverridesAndClears(t *testing.T) {
	...
	doJSON[oncall.RuleRequest](t, session, http.MethodPut, ruleURL(srv.URL, f.practiceID, f.engagementID),
	...
	if !panel.Rule.Overridden || panel.Rule.StartWeek != 39 || panel.Window.Start != octTwentyThree {
		t.Fatalf("panel = %+v, want a 39-week override opening 2026-10-23", panel)
	}

	doJSON[oncall.RuleRequest](t, session, http.MethodPut, ruleURL(srv.URL, f.practiceID, f.engagementID)+"?clear", oncall.RuleRequest{}, http.StatusOK)
	panel = getJSON[oncall.EngagementOnCall](t, session, panelURL(srv.URL, f.practiceID, f.engagementID), http.StatusOK)
	if panel.Rule.Overridden || panel.Window.Start != windowStart {
	...
	body := doJSON[map[string]any](t, session, http.MethodPut, ruleURL(srv.URL, f.practiceID, f.engagementID)+"?bad",
		oncall.RuleRequest{StartRule: &granted, StartWeek: &week}, http.StatusBadRequest)
	if details, _ := body["details"].(map[string]any); details[fieldWeek] != oncall.MsgWeekWithoutRule {
...
func TestEngagementRule_RefusesAPostpartumEngagement(t *testing.T) {
	...
	granted := oncall.StartAttachmentGranted
	doJSON[map[string]any](t, session, http.MethodPut, ruleURL(srv.URL, f.practiceID, f.engagementID),
