...
// on a fixed cadence (and ADR-0013's nudge on demand) to run
// Worker.ProcessPending, mirroring staffinvite.ProcessOutboxHandler's
// shape -- a separate endpoint per ADR-0010's accepted
// per-Notification-type cost, since this worker processes its own outbox
// table. There is no Staff or Client session on this path, so it
// authenticates the caller against secret instead. secret must be
// non-empty: an empty configured secret refuses every request rather
// than accepting an unauthenticated one.
func ProcessOutboxHandler(db *sql.DB, worker Worker, secret string) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// Licenses the two engagement_offers policies 00041 added for this
		// worker, and the practice_invitations one 00038 already added.
