...
**Practice**:
A tenant business. May be a solo doula or a multi-doula business with non-doula Staff. An Owner may **delete** it — a 30-day restorable window followed by an **Erasure** cascade across every Client still on file — see **Deletion** below. Decided on [#871](https://github.com/markgoho/doula-cloud/issues/871). See [ADR-0031](docs/adr/0031-practice-deletion-is-a-thirty-day-window-then-the-erasure-cascade.md).
...
_Client says_: my care ("Your care" as a heading). Never "my pregnancy" — too narrow for postpartum-only work, and wrong for a Client whose pregnancy ended in loss. Each status has one fixed Client label, the same for every Client: `intake` → Getting started, `active` → Ongoing, `completed` → Care ended. A status that cannot be labelled kindly for **every** Client means the status set is missing a value; that is a gap in the model, never a conditional label. Kind, the birth outcome and the ending reason are staff-only and have no Client word at all. Where two Engagements sit at the same Practice (ADR-0015 permits it) and need telling apart, the label adds when this one began — "started {date}", the Engagement's own `created_at`, never a due date or a birth date — because it is the one fact that stays true and kind for every Client, whatever became of the pregnancy. The register holds no finer-grained fact than the day: two Engagements begun at the same Practice on the same day carry the same label, and the status beside each on the portal root list is what separates them there. Decided on [#310](https://github.com/markgoho/doula-cloud/issues/310).
...
A Visit also carries its own free-text `notes`, written and re-written by any Staff member who may read that Visit, each edit recorded in the Activity log as its own action naming the acting Staff member — a per-Visit record, distinct from the Care Plan's per-Engagement structure, and staff-only in the same sense the Care Plan is (see [ADR-0006](docs/adr/0006-read-follows-the-role.md)). Decided on [#251](https://github.com/markgoho/doula-cloud/issues/251).
...
**Birth Plan**:
A standalone document of the Client's labor/delivery preferences, meant to be handed to a third party (e.g. hospital staff). Distinct from Care Plan. Structure is defined per-Practice via a Plan Template; staff-drafted, with a read-only view in the Client portal, a print stylesheet, and a PDF she can download -- rendered fresh from the plan's current answers on every request, never a stored snapshot, so a copy she already holds does not update after a later Staff edit, the same way a printed copy never has. The Practice side carries the same PDF, from the same rendering, so what she takes and what the Doula takes are one document. Decided on [#306](https://github.com/markgoho/doula-cloud/issues/306); scoped to Birth Plan only, since Care Plan has no Client-facing surface to extend it from. Offered on an Engagement whose kind calls for one (`kind = birth`) **and** which has a living or expected baby, per [ADR-0015](docs/adr/0015-three-facts-on-an-engagement-the-person-lives-in-the-login.md)'s standing rule -- so a loss, and an `unknown` outcome, both stop it being offered, while nothing recorded yet means the pregnancy is still expected. Two suppressors, decided on [#311](https://github.com/markgoho/doula-cloud/issues/311) and [#294](https://github.com/markgoho/doula-cloud/issues/294), and asked as one derived question in one place so no surface can disagree with another. Derived on every read rather than stored: nothing is flagged, archived or hidden on the Plan Instance, so a corrected outcome gives her the Birth Plan back with no separate act. She may acknowledge that she has read it — a v1, acknowledgement-only answer to "does she contribute to it" — but cannot edit a field or suggest a change; a Staff edit that changes the stored answers clears her acknowledgement, so it always means "read since it last changed." Decided on [#301](https://github.com/markgoho/doula-cloud/issues/301); a richer answer (suggest-and-approve, or Practice-defined client-editable fields) stays a candidate future ticket, tracked for signal on [#243](https://github.com/markgoho/doula-cloud/issues/243).
_Client says_: her Birth Plan, per the **Plan Instance** entry below. Where it does not apply, she meets no mention of it anywhere in the portal: no nav item, no link, and the address behaves as if it does not exist -- the same not-found screen any unreachable page shows her, never a Birth-Plan-specific word and never "not yet" for something that is never. Decided on [#311](https://github.com/markgoho/doula-cloud/issues/311). Where it does apply, "I've read this" is the one control she has over it; Staff's own view shows "Reviewed by client on {date}" or "Not yet reviewed by client". The model holds one acknowledged/not-acknowledged fact, not a history of edits and reads, so "not yet reviewed" is also what a Staff edit since her last read looks like — there is no separate word for "she read an earlier version."
...
**Erasure**:
Destroying a Client's personal data because she asked, honoring US state right-to-delete law. It is the one act in this product that leaves the record less complete on purpose, and it is not **ending** — an Attachment ends, an Engagement completes, a Contract is voided, and all three leave more of the record than they found. Erasure is the other thing. Only the **Owner** may run it, the same seat that throws the MFA switch. Her record is redacted where it sits and never deleted: the row keeps its id, so every Message, Contract, Visit, Invoice and Engagement Request that names her still resolves and the Practice keeps the financial and clinical record it is obliged to keep. Her **Activity** is not edited — it is shredded, by destroying the key its entries were sealed under, so the fact that something happened and who did it survives while what changed becomes unreadable. Outside Doula Cloud, every Stripe Customer she ever had is deleted at once — one per connected Stripe account since [#780](https://github.com/markgoho/doula-cloud/issues/780), plus any older one recorded on an Invoice from when each bill raised its own — and her transactions are redacted once Stripe will allow it, 90 days after they were taken; her portal login is deleted and the session holding it with it. What is *not* erased is free text a person typed by hand — a Message, a signed Contract's prose, a Plan Instance answer, a Visit's notes — and that is a stated limitation, not an oversight. Decided on [#394](https://github.com/markgoho/doula-cloud/issues/394). See [ADR-0027](docs/adr/0027-erasure-redacts-in-place-and-shreds-the-key.md).
_Client says_: delete my information — she asks the Practice, never the product; there is no button of her own.
_Avoid_: Deletion (nothing is deleted — the row and every record naming her stay; **Deletion** is the different, Practice-level word below), Anonymization (a stable pseudonym is a re-identification key against the free text erasure does not scrub), Ending, Closing, Archiving (all three are the opposite act: they preserve)

**Deletion**:
A **Practice** asking Doula Cloud, rather than a Client asking her Practice — the same shape as **Erasure** one level up, and a different word on purpose so the two never blur. An Owner starts a 30-day restorable window; during it the Practice is fully locked to every Staff and Client request, with one exemption: a Staff member's plain "who am I here" read, which routes every role to the pending-deletion screen — an Owner to restore it, everyone else to a notice that it is locked. If the window closes, Doula Cloud erases every Client still on file the same way a single Client's own Erasure works, forfeits any unspent **Credit** balance, and the act cannot be undone. The `practices` row keeps its id and its own fields — unlike Erasure, nothing on it is scrubbed, because a Practice is a business entity, not a natural person. Doula Cloud's own billing records (the Credit ledger, the Stripe Connect account and its payouts) are kept regardless, and the confirmation says so before an Owner commits. Decided on [#871](https://github.com/markgoho/doula-cloud/issues/871). See [ADR-0031](docs/adr/0031-practice-deletion-is-a-thirty-day-window-then-the-erasure-cascade.md).
_Avoid_: Erasure (that word stays a Client's own act), Closing, Deactivating (both imply a reversible pause; a finalized Deletion is not one)
