...
// Reads the same Engagement summary the hub's own `+page.ts` does
// (engagement-load.spec.ts), not the Birth Plan itself -- see this
// route's own `+page.ts` doc comment for why.
describe('engagements/[engagementId]/birth-plan/+page.ts load', () => {
	it('fetches the Engagement, for naming the Client and linking back', async () => {
		const detail = {
	...
	// ADR-0008's attachment rule: an unattached contractor Doula 403s here
