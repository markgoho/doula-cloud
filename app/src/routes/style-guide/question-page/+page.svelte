<script lang="ts">
	...
	const steps: JourneyStep[] = [
		{ label: 'Who Sarah is', href: '/style-guide/question-page#name', status: 'completed' },
		{
			label: 'How to reach Sarah',
			href: '/style-guide/question-page#reach',
			status: 'current',
			questions: [
				{ label: 'Email address', href: '/style-guide/question-page#email' },
				{ label: 'Phone number', href: '/style-guide/question-page#phone' }
			]
		},
		{ label: 'Where Sarah lives', href: '/style-guide/question-page#lives', status: 'todo' },
...
{#snippet errorSummary()}
	<Notice message="There is a problem. Enter an email address." variant="error" />
{/snippet}
...
{#snippet actions()}
	<Button label="Continue" type="submit" onClick={noop} />
...
{#if isLabelMode}
	<QuestionPage
		...
		caption="How to reach Sarah"
		question={{ as: 'label', text: "What is Sarah's email address?", for: 'client-email' }}
		hint="Optional. Without an email address Sarah cannot be invited to the client portal and cannot be invoiced."
		content={emailInput}
		{actions}
	/>
{:else}
