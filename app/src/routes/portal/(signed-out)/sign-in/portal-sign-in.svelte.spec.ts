 ...
 * mock so a test can change what the second press gets.
 */
...
describe('Redeeming a sign-in link over a live Staff session (#610)', () => {
	it('warns on the button instead of signing her in', async () => {
		...
		expect(redeem.mock.calls[0][1].headers['X-Confirmed']).toBeUndefined();
	});

	it('sends the same redeem again, confirmed, when she presses through', async () => {
		const redeem = await reachWarning();
		redeem.mockResolvedValue(jsonResponse({ ok: true }));
		apiFetchWithSession.mockResolvedValue(
			jsonResponse({ engagements: [{ engagementId: 'engagement-1', practiceName: 'Bright Beginnings' }] })
		);
		...
		expect(redeem.mock.calls[1][1].headers['X-Confirmed']).toBe('true');
	});
