...
**What did not change.** Every page Hugo served is served at the same path. **Amended by [#1475](https://github.com/markgoho/doula-cloud/issues/1475):** the paths have no trailing slash, `/pilot-terms` and `/p/<slug>`. The switch first copied Hugo's `/p/<slug>/` form, but `website.HostedPageURL` gives Stripe `/p/<slug>` ([#382](https://github.com/markgoho/doula-cloud/issues/382)), so each Stripe visit started with a 301. The build now writes `<path>.html`, Firebase Hosting serves it at `<path>` (`cleanUrls`), and the slash form answers with a 301 to it (`trailingSlash: false`). The [#443](https://github.com/markgoho/doula-cloud/issues/443) probe requests the same no-slash URL that Stripe holds. The Practice page keeps `data-practice-page`. `scripts/sync-practice-pages.ts` keeps its `SYNC_PRACTICE_PAGES=required` gate and prunes before it writes; it now writes one JSON file per page into `site/practice-pages/`. The `repository_dispatch` rebuild is unchanged. The site still publishes no RSS feed, no taxonomy pages and no `/p/` index.
