...
**Staff** — doulas, Practice Admins, Practice Owners — are in the product most working days, often across a laptop shared at a birth center and a personal phone. They read other people's health records, so a compromised Staff account costs the most.
...
- **The shared birth-center laptop turns it from a convenience into a hazard.** A "Sign in with Google" button in a browser already signed into someone's Google account is one click with no prompt, so the fastest path through the door is signing in as whoever used it last. That is a wrong-person hazard in a product holding other people's health records.
...
**The identifier is a prefixed UUID, and the prefix is the sanctioned way to tell the two populations apart.** `client_portal_users.identity_uid` is `text`, so nothing changes shape. The alternative — a tier column on `sessions` — is rejected: the prefix is not a hint about the identifier, it *is* the namespace it was issued in, total and deterministic because every uid is minted by exactly one of two issuers. A tier column would be a second place holding the same fact, free to disagree with the first. Three things read it, which is the argument for naming it here rather than leaving it to a build: the session-lifetime branch (30 days rolling against 12 hours), the eviction check below, and the request boundary added by the amendment at the end of this section.

**Amendment ([#1024](https://github.com/markgoho/doula-cloud/issues/1024)): a session is a credential for the population it was issued in, and for no other, and `authn.Begin` is where that is enforced.** This ADR always implied it — two populations, different identifier namespaces, a Portal Account legible only from a portal session — and nothing in the code read the prefix at the request boundary. `sessions` holds both populations' rows, so a live Client portal session reached every `/api/staff/*` route mounted outside `staffauth.Middleware`, and a live Staff session reached every `/api/portal/*` route. Each was caught only downstream, by whichever handler happened to look its caller up in `staff` or `client_portal_users` and find nothing: fail-closed by accident rather than by design, and `POST /api/staff/verify-email/request`, which looked nothing up, minted a token and queued an outbox row for a Client.

`authn.Begin` now takes the `Tier` its caller serves, and refuses a session issued in the other one. Every call site has to name a population to compile, which is what makes the rule total rather than something each new route must remember. **The refusal is the one a cookie naming no live session already gets** — 401, same words — because any distinguishable answer would tell a caller which namespace her cookie was issued in, which is the non-disclosure rule the eviction section below already states. Everything already behind a population's own middleware reaches `Begin` and is covered by the same argument, since that middleware is itself one of the call sites. Two groups of route are untouched, because neither reads a session at all: the bootstrap endpoints, whose Identity Platform ID token cannot carry `portal_` (that character is outside the alphabet Identity Platform mints uids from), and the pre-account endpoints, whose credential is the emailed link's own single-purpose token.

**The database's own fail-closed lookups stay**, unchanged, for the same reason migration `00006`'s guards stay: the backstop is not where a saving is spent.
...
Identity Platform has **no WebAuthn support**, so passkeys would be a BFF-owned build — which ADR-0004 makes cheap, since a successful assertion calls the same `authn.MintSession` that mints `__session` today. The shared birth-center laptop does **not** break them: cross-device QR and Bluetooth transport is the flow FIDO's own UX guidance recommends for exactly that case, and the private key never lands on the shared machine, subject to confirming Bluetooth on real pilot hardware.
