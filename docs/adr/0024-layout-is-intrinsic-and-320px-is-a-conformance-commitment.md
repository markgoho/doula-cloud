...
1. **Reach for the query-free answer first.** `flex-wrap` with a real `flex-basis`, `repeat(auto-fit, minmax(<content floor>, 1fr))`, `clamp()`, and the intrinsic sizing keywords `min-content`, `max-content` and `fit-content` between them handle most adaptation with no query at all. This is the majority case, not the clever case, and it is the only mechanism that produces a **quantum layout** with no authored configuration in it: the browser resolves the arrangement again and again across the continuum and nobody wrote down a single one of those moments.
   ...
   **A wrap is not observable, so never author a query to predict one.** When rule 1's own mechanisms adapt — a `flex-wrap` line breaking, an `auto-fit` track count changing — *no selector reports that it happened*. A query placed next to one is therefore not reading the layout; it is a second opinion about a decision the browser already made, kept in agreement by hand. It will drift, and nothing will fail when it does, because there is nothing to compare it against.

   Measured, on [#585](https://github.com/markgoho/doula-cloud/issues/585): `StepRail` chose between two presentations with `@container (min-width: 67.5rem)` while its host laid it out with `sidebar-l`. The flex line actually pairs at **1078px**, and on `CheckAnswers` in its wide state at **955px** — a 125px band in which the layout opened a rail column and the component put a progress bar in it and left the rest empty. The 67.5rem literal had been copied from a 60rem query both hosts lost to `sidebar-l` on [#564](https://github.com/markgoho/doula-cloud/issues/564); it outlived its own mechanism and no check noticed, because the coupling that appeared to hold it was never registered.

   What to do instead, in order: let the component be correct in both arrangements, so nothing has to know which one happened; or give the reader the choice with an HTML disclosure, which is what #585 did; or have the host pass what it *knows* — what the page is for — never what it is guessing the browser did with it. Reach for a query only where a genuine content question exists, per rule 2.
...
4. **A containment context declares the base size on its children, never on itself** — **the pairing** in `CONTEXT.md`. Added by [#544](https://github.com/markgoho/doula-cloud/issues/544). A container unit resolves against the nearest *ancestor* container, so an element that declares a container never resolves against its own; and `font-size` inherits as a computed length rather than as the token, so text that merely inherits carries the size computed *outside* the container it now sits in. Declaring a container and leaving the size to inheritance therefore produces the one thing rule 2 exists to prevent — a component answering the window from inside a rail — with no query anywhere in sight.

   Measured: at a 1440px window with the drag surface's frame at 320px, a `<dd>` in `Description list` rendered at **17.1px**, the size for a 1425px container, where the token declared one level down gives **15px**. `Description list` needed **452px** inside its 320px frame; with the pairing in place it needs **399px**. The 53px difference is text that never asked its container how much room it had.

   The pairing goes on the container's children because that is the only place it can go: an element cannot re-resolve a container unit against a container it declares itself. `body > *` (`base.css`) carries it for the whole app, and each of the other contexts carries its own. It is gated in source rather than by rendering, for the reason recorded on rule 3 of `layout.usage.spec.ts`: the continuum check's own browser window is about 414px wide, so inside and outside are nearly the same number there and the defect is invisible to it.
...
A labeled scroll region remains correct for genuinely tabular numeric data, where reading across matters more than reading a record. WCAG 1.4.10 explicitly exempts *"data tables (not individual cells)"* from its ban on two-dimensional scrolling, so this is conformant; when used it carries `tabindex="0"`, `role="region"` and an accessible name, without which a keyboard user cannot reach the scroll at all.
