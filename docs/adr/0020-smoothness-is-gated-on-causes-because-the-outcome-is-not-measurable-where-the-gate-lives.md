...
- **Dense lists** are bounded rather than timed. [`DataTable.usage.spec.ts`](../../app/src/lib/components/organisms/DataTable.usage.spec.ts) asserts no route hands `DataTable` an unbounded list, and [`DataTable.performance.svelte.spec.ts`](../../app/src/lib/components/organisms/DataTable.performance.svelte.spec.ts) caps a row at six elements and asserts the row array itself is read a fixed, small number of times per row. A read count, not a millisecond budget: it catches per-row work that reads the rest of the list, which is what actually makes a list stutter, and — unlike the mount-time ratio this replaced (see the 2026-09-03 amendment below) — it means the same thing on any machine because nothing about it is a clock reading.
...
- **Focus visibility and keyboard reachability** are handed to accessibility outright ([#447](https://github.com/markgoho/doula-cloud/issues/447)) rather than checked twice under two names. Focus *return* — where focus lands after a dialog closes — is a sequence no snapshot scanner can see, so it becomes a required behavior of the Dialog component's own spec when a Dialog is first built.
- **No database fixture is built for this**, and the pilot's 14-doula agency is not seeded. Scroll and render cost is pure client-side work, so a seeded Postgres would add minutes to every run and prove nothing the row count does not; `app/e2e/stack.ts`'s `execSQL` also spawns one `compose exec` subprocess per statement, which makes a 350-Client seed expensive. In-memory row arrays at 200 and 800 are what the budgets run against, and they are sufficient because the assertions are a ratio and an element count, not a wall-clock number. The agency-scale figures — roughly 14 Staff, 350 Clients, 450 Engagements, 40 ledger events per Engagement, 20 Practice-defined fields — belong to [#446](https://github.com/markgoho/doula-cloud/issues/446), where page size is the actual question.
...
- **Per-row work performed on a private copy of the list.** The read-count instrument that replaced the mount-time ratio (below) only sees reads of the array actually passed in as `DataTable`'s `rows` prop. A `$derived(rows.toSorted(cmp))` or similar reads that array once, up front, and returns a plain array with no counting attached — any per-row cost paid against *that* copy is invisible to the gate. Real, but accepted: a sort or dedup added to `DataTable` would change the read counts the gate already asserts and force a conscious update, which is the same property `costPerRow`'s exact-equality check already relies on for element counts.

## Amendment, 2026-09-03: the last wall-clock assertion left the gate ([#489](https://github.com/markgoho/doula-cloud/issues/489))

The "Dense lists" bullet above originally asserted a mount-time ratio — 800 rows taking under a threshold times as long as 200 — reasoning that per-row work reading the rest of the list turns 4x the rows into 16x the time. It was the one wall-clock measurement left after this ADR moved everything else to space, and it flaked: 6.66, 6.67 and 6.57 against a threshold of six (all on commits that touched no front-end code), which moved the threshold to ten in commit `f989f6d` — without amending this ADR, the exact quiet-edit failure mode this decision exists to prevent. #489 then found 7.19 against the new threshold of ten, also on an unrelated commit.

Before changing the number again, #489 measured the spread deliberately: four reruns of one identical commit on CI read 2.75, 4.01, 6.94 and 6.97 — a 2.5x range on the *same* build. Combined with the historical readings, the ratio wanders across roughly 2.7 to 7.2, which leaves very little headroom below the ~16x the assertion exists to catch. No threshold in that gap would have been trustworthy for long.

The replacement is a countable signature for the same failure. `DataTable.performance.svelte.spec.ts` now wraps the `rows` array in a `Proxy` that counts every `get` trap — every index read, every `.length` check — while mounting. Measured at 200/400/800 rows: exactly 2 reads per row, zero variance, every run, because the array is indexed once for the table view and once for the record view (ADR-0024) and nothing else touches it. A column whose accessor calls `rows.indexOf(row)` — the shape of the failure this gate exists to catch — reads the array roughly n times per row instead of 2, which the spec proves directly with such a column rather than assuming it: at 200→800 rows it clears the gate's bound by two orders of magnitude. The gate is now a read count with a small constant bound, not a millisecond budget, and it means the same thing whether CI's runner is contended or idle.
