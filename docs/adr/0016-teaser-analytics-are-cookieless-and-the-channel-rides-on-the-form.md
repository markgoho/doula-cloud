...
**What this means for the DPA, and it belongs to [the business map](https://github.com/markgoho/doula-cloud/issues/375) rather than here.** A DPA covers an *account*, not a site. This account is personal today and serves eight sites, so signing Pirsch's DPA ([#392](https://github.com/markgoho/doula-cloud/issues/392)) signs it as an individual for all of them — which is correct now, because the LLC does not exist yet ([#377](https://github.com/markgoho/doula-cloud/issues/377) is unfiled).

The sites are **not** unrelated: several are intended to generate revenue and to roll up to the same entity. That makes one account under the LLC, covered by one DPA in the LLC's name, the right end state — not a split. The whole account transfers, once there is an entity to transfer it to, and every site on it is covered by the same agreement. It is the shape [#376](https://github.com/markgoho/doula-cloud/issues/376) already chose: a neutral holding name with Doula Cloud run as a DBA, deliberately leaving room for more than one product.

Nothing needs doing before January. The teaser runs on a personal account under a personally-signed DPA, and the transfer happens once the entity exists — the same handover as the Google Cloud BAA ([#336](https://github.com/markgoho/doula-cloud/issues/336)) and taking Stripe to production under the real entity ([#387](https://github.com/markgoho/doula-cloud/issues/387)).
