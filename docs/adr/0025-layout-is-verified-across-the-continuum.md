...
- **A route joins the same way, but not through this mechanism.** The component directory above is the drag surface's own territory, and a route cannot join the drag surface at all: the drag surface is a page a browser is actually running, and `vi.mock` does not exist there — nothing can substitute a route's `$app/state`/`#lib/api.js` calls the way a test file's mock can (`route-continuum.svelte.spec.ts`'s own header comment works through this in full). A route instead declares a `page.fixture.ts` beside its `+page.svelte`, and `route-continuum.svelte.spec.ts` discovers it the same way the component check discovers a component fixture: a route with neither a `page.fixture.ts` nor an entry in that file's own `UNSWEPT` list fails the build, so coverage cannot quietly stop growing with the app. [#595](https://github.com/markgoho/doula-cloud/issues/595) is what emptied `UNSWEPT` for the first time. This is mechanism, not design — it names no width, the same as the check itself.
