...
- **`staff_email_change_outbox` needed no recheck at all**, and has none. It resolves no Staff person at send time, so it cannot hit `ErrAccountNotFound` — and it is the table resolve-at-source deliberately skips, for the reason above.
- **A password reset queued *after* the deletion cannot exist.** `RequestResetHandler` resolves the address through Identity Platform, and her account is gone, so there is nothing to queue. The narrow case neither mechanism covers is a reset request that was already in flight and commits after the deletion does; that row dead-letters. No mail is sent either way — only the recorded status differs — and buying the last case would mean a worker reading absence of a `staff` row as deletion, which on this table would silently swallow a Client Portal account's mail.

## The endpoint is self-only, by shape

`DELETE /api/staff/account` — the `/api/staff/...` family with no `{practiceId}` in it, beside `PUT /api/staff/work-state` and `DELETE /api/staff/mfa`. It carries no staff id anywhere in the route or the body, so there is no path parameter a caller could redirect at another person and no authorization branch to get wrong. The row acted on is the one the session cookie's identity resolves to. Migration 00101's `staff_self_login_deletion` policy is the same rule again at the boundary that can enforce it: it admits the caller's own row, in the pre-Practice window, leaving only with `deleted_at` stamped and `identity_uid` holding the sentinel it spells out in full.
