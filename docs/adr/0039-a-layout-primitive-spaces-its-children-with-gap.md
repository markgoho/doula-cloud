...
- **A column flex container stretches its items across the inline axis**, where block layout let an inline-level child shrink-wrap. `DataTable`'s `.table-view` is the one place this bit: #542 deliberately removed `inline-size: 100%` from it so a table stops where its columns are satisfied, and stretching put that width back by another route. A child that must keep its intrinsic width says `align-self: start` at the call site.
