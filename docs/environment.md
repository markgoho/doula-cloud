...
#339 (the Staff invitation Notification, RA-G1) reuses `NOTIFICATION_WORKER_SECRET` for a sixth endpoint, `/api/internal/notifications/process-staff-invite-outbox` -- same secret, same header, its own Cloud Scheduler job, its own outbox table (`staff_invite_outbox`). No write site queues that table yet (#316 builds the `InviteHandler` that will), so there is nothing for the endpoint to send even once a job is provisioned; left unset for the same reason as the others.
