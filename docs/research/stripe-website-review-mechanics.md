...
Absences were checked mechanically, not by recollection. Every page named in the roster at the end of this document was fetched to a local file — support-center articles as their extracted source Markdown, docs pages as their `.md` form, the Services Agreement as stripped HTML text — concatenated, and searched case-insensitively for the term lists quoted in each absence below. Where a term does occur but not in the sense at issue, the hit is named rather than suppressed.
...
## The eight answers, in one table
...
| 8. Which business name must the website show? | Stripe holds the legal name (`company.name`) and the trade name (`business_profile.name`) as separate fields, verifies only the legal one against the IRS, and asks the website for "your business name" without qualifying it. A DBA-only site satisfies Stripe, provided `business_profile.name` is set to that DBA. | Read first-party, with a checked absence on any legal-name-on-website requirement |
...
**Checked absence: Checkout and hosted onboarding.** No page in this sweep states that `business_profile.url` is rendered on a Stripe Checkout page or inside Stripe-hosted Connect onboarding. [Stripe-hosted onboarding](https://docs.stripe.com/connect/hosted-onboarding) says the form's appearance is customized from the platform's Connect settings — "your brand's name, color, and icon" — and mentions `business_profile.url` only as something the platform may *prefill on the connected account*: "If you onboard an account and your platform provides it with a URL, prefill the account's `business_profile.url`. If the business doesn't have a URL, you can prefill its `business_profile.product_description` instead." Pages read for this: the hosted-onboarding doc, the receipts doc, the activation doc, the Account object reference, and [Customize checkout for Payment Links](https://docs.stripe.com/payment-links/customize).

The Checkout half of that absence is worth stating precisely, because Checkout demonstrably renders *some* of the account's public details. [Customize checkout for Payment Links](https://docs.stripe.com/payment-links/customize) names exactly which: a terms-of-service URL and a privacy-policy URL, both drawn from the account's Public details — "The checkout page also links to your privacy policy if you set a privacy policy URL in your account's Public details" — plus, under "Set store policies and contact information", "your return, refund, and legal policies on the payment page in addition to your support contact information", configured from the Checkout and Payment Links settings. The business website URL is not in either list. So the finding is not that Checkout shows nothing public; it is that Stripe enumerates what Checkout shows and `business_profile.url` is not among the enumerated fields.
...
**Checked absence: search-engine indexability is not among Stripe's requirements anywhere.** The terms *index*, *indexed*, *indexable*, *noindex*, *robots*, *robots.txt*, *X-Robots-Tag*, *SEO*, *search engine*, *Google*, and *discoverable* appear in none of the pages listed in section 3, with the single exception of the *Sitemap* footer link on the stripe.com page that serves the Services Agreement. They specifically do not appear in the Business website FAQ, the Website checklist, [Set up your account](https://docs.stripe.com/get-started/account/activate), or the seventeen-member `invalid_url_*` error family. Stripe enumerates its failure modes in that enum to a fine grain — down to which individual policy is missing — and an indexing failure mode is not in it.
...
**Inference, labeled as such — this is the reading, not Stripe's.** An invite-only pilot offered by email, on terms different from the eventual general-availability terms, is an offer with conditions attached: who is eligible, what it costs during the pilot, how long the pilot pricing lasts, what happens at the end of it, and how a participant leaves. That is materially what Stripe's "promotion, discount, or trial" describes, and it is exactly the situation the card-network rationale is aimed at — a customer who agrees to something whose conditions were never written down is the customer who disputes the charge later. The narrow reading, that a promotion means a public discount code or a public sale, is not supported by the word "trial" sitting in Stripe's own list.
...
## 8. Legal name and DBA: Stripe holds both, and the website needs only one of them

The question is whether a site branded solely with the assumed name satisfies the FAQ's "your business name" and its "must align with the information you've provided to Stripe", or whether the legal entity has to appear too.

### 8.1 The name fields, and what each one is

**Read first-party**, from the API reference and the Dashboard-facing support articles.

| Field | Stripe's own description | Legal or trade name | Where it surfaces |
| --- | --- | --- | --- |
| `company.name` | "The company's **legal** name." | Legal registered name | Dashboard → Settings → Business → Business Details, labeled **Legal Name**; verified against the IRS |
| `business_profile.name` | "The **customer-facing** business name." | Trade name — this is where a DBA lives | Grouped by Stripe with the public support fields; the activation page's "Business name" that customers see on statements or receipts |
| `settings.payments.statement_descriptor` | "The default text that appears on statements for non-card charges outside of Japan." | Neither, exactly — a 5–22 character string that must *reflect* one of the names | The customer's bank or card statement |
| `settings.card_payments.statement_descriptor_prefix` | "Default text that appears on statements for card charges outside of Japan, prefixing any dynamic `statement_descriptor_suffix` specified on the charge." | Same | Same, as the static half of a dynamic descriptor |
| `settings.dashboard.display_name` | "The display name for this account. This is used on the Stripe Dashboard to differentiate between accounts." | Neither; an internal label | Stripe Dashboard only |
| `identity.business_details.registered_name` (v2 Accounts) | "The business legal name." | Legal registered name | The v2 equivalent of `company.name` |
| `display_name` (v2 Accounts) | "A descriptive name for the Account. This name will be surfaced in the Stripe Dashboard and on any invoices sent to the Account." | Trade-name-shaped | Dashboard and Stripe's own invoices to the account |

The split is clean and Stripe states it in two words: `company.name` is *legal*, `business_profile.name` is *customer-facing*. They are separate fields and both exist on the same account.

**Checked absence: Stripe has no field named DBA.** The strings `dba` and `doing_business_as` occur zero times in the v1 Account object reference — read in its expanded `?query=business_profile`, `?query=company`, `?query=settings` and `?query=requirements` forms — and zero times in the v2 Core Accounts object reference read in its expanded `?query=identity` form. Stripe uses the *concept* constantly in prose ("Reflects your Doing Business As (DBA) name") but exposes no API field for it. **Inference, labeled:** the assumed name is therefore held in `business_profile.name`, because that is the only field Stripe describes as customer-facing and it is the field Stripe's statement-descriptor matching has to be reading when it accepts a DBA. Stripe does not say this in so many words.

### 8.2 Yes, Stripe collects both, and it verifies the legal one against the IRS

**Read first-party.** [Tax ID verification](https://support.stripe.com/questions/tax-id-verification) states the check and names the field:

> Go to the Business Details section of your Dashboard under **Settings** > **Business**. Here you can modify your Tax ID information including Type of Business, **Legal Name**, and Tax ID.
>
...
The API reference carries the same check as three enumerated failure codes on `requirements.errors[].code`: `verification_failed_tax_id_match` — "The tax ID on the account cannot be verified by the IRS. Either correct any possible errors in the company name or tax ID, or upload a document that contains those fields" — `verification_failed_tax_id_not_issued`, and `verification_failed_name_match` — "The company name on the account could not be verified. Correct any errors in the company name field or upload a document that includes the company name."

So the field verified against IRS records is `company.name`, paired with the EIN. `business_profile.name` is not part of that check, and no Stripe page in this sweep describes any government verification of it. The two names are collected separately, held separately, and treated differently: one is matched against a federal database, the other is published to customers.

### 8.3 A site branded only with the DBA satisfies what Stripe states

**The FAQ's requirement is "Your business name", unqualified.** It does not say legal name, registered name, or entity name. Contrast that with [Receipts and paid invoices](https://docs.stripe.com/receipts), which *does* qualify it when it means the legal one: "For compliance reasons, some contact information is always required on receipts: **Legal business name**, Customer support address, Customer support email, Privacy policy URL." Stripe writes "legal business name" when it means the legal business name. The website FAQ does not.

**And that receipts line is the reason the website does not have to carry it.** The legal entity is not hidden from customers — Stripe puts it on every receipt itself, as a compliance requirement of its own, whatever the site says. A customer who pays a business trading as "Doula Cloud" sees the legal name on the Stripe receipt without the website ever printing it. The disclosure Stripe cares about is already made, on a surface Stripe controls.

**Checked absence: Stripe nowhere requires the legal entity to be disclosed on the business website.** The terms *legal name*, *legal business name*, *registered name*, *legal entity*, *entity name*, and *DBA* were searched across the Business website FAQ, the [Website checklist](https://docs.stripe.com/get-started/checklist/website), [Do I have to have a business website to sign up for Stripe?](https://support.stripe.com/questions/do-i-have-to-have-a-business-website-to-sign-up-for-stripe), [Website ownership verification](https://support.stripe.com/questions/website-ownership-verification-during-stripe-account-application), [Business information requirements to use Stripe](https://support.stripe.com/questions/business-information-requirements-to-use-stripe), [Set up your account](https://docs.stripe.com/get-started/account/activate), and the whole `invalid_url_*` error family. None of them attaches a legal-name requirement to the website. The Website checklist's closest item is about the *address* and is framed as optional credibility, not compliance: "Your business address — If you have a physical address, listing it on your website adds credibility for your business."

**"Align with the information you've provided to Stripe" is satisfied by the DBA, because the DBA is information provided to Stripe.** `business_profile.name` is a field on the account, the operator fills it in, and Stripe describes it as the customer-facing name. A site branded "Doula Cloud" aligns with a `business_profile.name` of "Doula Cloud". The corresponding failure code is `invalid_url_website_business_information_mismatch` — "The business information on your website must match the details you provided to Stripe" — and a site whose branding matches a name the account holds is not a mismatch on any reading of that sentence.

**Conclusion.** The operator's position is correct as far as Stripe is concerned, with one condition that has to actually be met: `business_profile.name` must be set to the assumed name, so that the name the site shows is a name the account holds. If `business_profile.name` were left null or set to the legal entity while the site said something else, the alignment argument disappears and `invalid_url_website_business_information_mismatch` becomes live. This is a settings question, not a website question.

Two things this finding does **not** cover, flagged because they are where the real exposure sits and neither is Stripe's rule. Consumer-protection and state assumed-name statutes have their own disclosure requirements, and card-network rules — which the Website checklist explicitly defers to, "it's your responsibility to ensure that you're compliant with these rules, including the Visa Rules and Regulations, the Mastercard Rules…" — are not reproduced by Stripe and were not read here. Nothing above is a finding about either. A footer line naming the legal entity costs one line of markup and removes both questions; Stripe simply does not require it.

### 8.4 Trading under a DBA is expected, not a risk signal

**Read first-party, and the evidence is that Stripe builds for it.** Three places treat legal-name-versus-trade-name divergence as ordinary:

- The statement descriptor rule accepts either. [What is a statement descriptor and how do I update it?](https://support.stripe.com/questions/what-is-a-statement-descriptor-and-how-do-i-update-it): the descriptor "must reflect your Doing Business As Name (DBA), URL, or Legal Entity Name", and "In the US, Stripe employs a matching algorithm to determine if the statement descriptor reflects your DBA, URL, or Legal Entity Name. The algorithm will accept exact matches, most acronyms, most matching words, and most substrings." A three-way accepted set is not how a risk signal is written. The API says the same thing in code: `invalid_statement_descriptor_business_mismatch` reads "The statement descriptor must be similar to your **business name, legal entity name, or URL**", and `invalid_statement_descriptor_prefix_mismatch` repeats the disjunction. Stripe's own enum treats the trade name and the legal name as interchangeable satisfiers.
- The bank-account rule accepts either. Stripe publishes a support article titled "Company name (legal or DBA) and bank account owner name are different", which exists precisely because the two commonly differ.
- The document list accepts either, and names the DBA statement as the instrument that reconciles them (see 8.5).

**Checked absence: no Stripe page in this sweep describes a legal-name-to-branding mismatch as a risk factor.** Searched: [Risk management for platforms](https://docs.stripe.com/connect/risk-management), [Identity verification](https://docs.stripe.com/connect/identity-verification), [Business information requirements to use Stripe](https://support.stripe.com/questions/business-information-requirements-to-use-stripe), [New Stripe account approval process](https://support.stripe.com/questions/new-stripe-account-approval-process), the Business website FAQ, and the `requirements.errors` enum. The enum's mismatch code, `invalid_url_website_business_information_mismatch`, is about the website disagreeing with the account, not about the account's two names disagreeing with each other.

**The one real dependency, and it is worth stating plainly.** Stripe's business-information review compares the website against the account. It does not compare the website against the state's corporate register. What it does compare against a government record is `company.name` against the IRS, and that comparison never touches the website at all. Those are two independent checks and the DBA site sits inside neither's failure mode.

### 8.5 Stripe asks for DBA evidence only on the document path, and it asks for a state filing

**Checked absence at activation.** No page in this sweep says a DBA must be registered before it can be used as `business_profile.name` or as the statement descriptor, and none says evidence of the registration is collected at activation. The statement-descriptor requirements list is five bullets long — Latin characters, 5–22 characters, at least one letter, no disallowed special characters, and reflects the DBA, URL or legal entity name — and registration is not one of them. Pages searched: [Statement descriptors](https://docs.stripe.com/get-started/account/statement-descriptors), [What is a statement descriptor and how do I update it?](https://support.stripe.com/questions/what-is-a-statement-descriptor-and-how-do-i-update-it), [Set up your account](https://docs.stripe.com/get-started/account/activate), [Business information requirements to use Stripe](https://support.stripe.com/questions/business-information-requirements-to-use-stripe), the Business website FAQ, and the Account object reference. What Stripe applies instead is the US matching algorithm quoted in 8.4, which is a string comparison against names the account already holds — not a registry lookup.

**Where Stripe does ask, it asks for the filing.** [Supporting documents to further review the account](https://support.stripe.com/questions/supporting-documents-to-further-review-the-account) lists, under "For U.S. merchants only":
...
That is the New York Certificate of Assumed Name by another name, and it is asked for *in addition to* an EIN verification letter or a state tax registration certificate, not instead of one. The same page sets the matching rule for any document: "The information in these documents, for example business name, business address, company registration number… should match the information on your Stripe account."

**Conclusion.** Registration is not a precondition Stripe checks up front, but the filed certificate is exactly the document Stripe names if the account is escalated to manual review. Having it on hand costs nothing. **Inference, labeled:** Stripe does not say a DBA statement is *required* of every US merchant trading under an assumed name — only that it is the accepted document when a further review happens.

## The pages that were read

Recorded so every checked absence above is auditable.

Stripe support center: [Business website for account activation FAQ](https://support.stripe.com/questions/business-website-for-account-activation-faq) · [Do I have to have a business website to sign up for Stripe?](https://support.stripe.com/questions/do-i-have-to-have-a-business-website-to-sign-up-for-stripe) (also served at `information-required-on-your-business-website-to-use-stripe`) · [Website ownership verification during Stripe account application](https://support.stripe.com/questions/website-ownership-verification-during-stripe-account-application) (also served at `domain-verification-during-stripe-account-application`) · [How to update your business website URL in the Dashboard](https://support.stripe.com/questions/how-to-update-your-business-website-url-in-the-dashboard) · [New Stripe account approval process](https://support.stripe.com/questions/new-stripe-account-approval-process) · [Accepting payments for pre-orders](https://support.stripe.com/questions/accepting-payments-for-pre-orders) · [Business information requirements to use Stripe](https://support.stripe.com/questions/business-information-requirements-to-use-stripe) · [Tax ID verification](https://support.stripe.com/questions/tax-id-verification) · [What is a statement descriptor and how do I update it?](https://support.stripe.com/questions/what-is-a-statement-descriptor-and-how-do-i-update-it) · [Supporting documents to further review the account](https://support.stripe.com/questions/supporting-documents-to-further-review-the-account) · [2024 updates to US verification requirements FAQ](https://support.stripe.com/questions/2024-updates-to-us-verification-requirements-faq)

Stripe docs: [Website checklist](https://docs.stripe.com/get-started/checklist/website) · [Set up your account](https://docs.stripe.com/get-started/account/activate) · [Statement descriptors](https://docs.stripe.com/get-started/account/statement-descriptors) · [Receipts and paid invoices](https://docs.stripe.com/receipts) · [Stripe profiles](https://docs.stripe.com/get-started/account/profile) · [Stripe-hosted onboarding](https://docs.stripe.com/connect/hosted-onboarding) · [Customize checkout for Payment Links](https://docs.stripe.com/payment-links/customize) · [Identity verification](https://docs.stripe.com/connect/identity-verification) · [Risk management for platforms](https://docs.stripe.com/connect/risk-management) · [Review and take action on connected accounts](https://docs.stripe.com/connect/dashboard/review-actionable-accounts)

Stripe API reference: [The Account object](https://docs.stripe.com/api/accounts/object), read in the expanded forms `object.md?query=business_profile`, `object.md?query=company`, `object.md?query=settings` and `object.md?query=requirements` so that every nested field description and the complete `requirements.errors[].code` enum were in view; and [The v2 Account object](https://docs.stripe.com/api/v2/core/accounts/object), read expanded as `object.md?query=identity`.
...
8. **Set `business_profile.name` to the DBA before submitting.** It is the field that makes the DBA-only site defensible: Stripe's alignment test compares the site against the account, and the account has to actually hold the name the site shows. The legal entity goes in `company.name`, where the IRS check reads it and from where Stripe puts it on every receipt; the website is not asked to repeat it.
9. **Treat the declared URL as permanent infrastructure.** Stripe "regularly checks" it, and payouts are the stated lever when it cannot verify. If the January 2027 site restructures the domain, the declared URL has to keep resolving to compliant content or be updated in the Dashboard at the same time.
