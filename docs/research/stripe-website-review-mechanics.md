...
## 8. Legal name and DBA: Stripe holds both, and only one of them is public
...
**Conclusion.** Registration is not a precondition Stripe checks up front, but the filed certificate is exactly the document Stripe names if the account is escalated to manual review. Having it on hand costs nothing and is the difference between a same-week resolution and a stalled one. **Inference, labelled:** Stripe does not say a DBA statement is *required* of every US merchant trading under an assumed name — only that it is the accepted document when a further review happens.
