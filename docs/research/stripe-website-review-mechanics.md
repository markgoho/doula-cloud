...
| --- | --- | --- | --- |
| `company.name` | "The company's **legal** name." | Legal registered name | Dashboard → Settings → Business → Business Details, labelled **Legal Name**; verified against the IRS |
...
**Conclusion.** Registration is not a precondition Stripe checks up front, but the filed certificate is exactly the document Stripe names if the account is escalated to manual review. Having it on hand costs nothing. **Inference, labelled:** Stripe does not say a DBA statement is *required* of every US merchant trading under an assumed name — only that it is the accepted document when a further review happens.
