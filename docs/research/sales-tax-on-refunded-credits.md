...
3. **The recovery dies three years after the tax was payable.** For a sale in the 1 March – 31 May quarter, the ST-100 is due 20 June and the recovery closes on 20 June three years later. A no-time-limit refund promise therefore guarantees a window — opening between **three years and 20 days** and **three years, three months and 19 days** after each sale, depending where in the quarter it fell — in which the seller must hand back tax it can never get back. **This is a decision, not a finding**; it is set out in full in section 4.
...
| --- | --- | --- | --- | --- |
| 1 March 2027 | 1 Mar – 31 May 2027 | 20 June 2027 | **20 June 2030** | 3 years, 3 months, 19 days |
...
So: **the window in which the seller must refund tax it can no longer recover opens between three years and 20 days, and three years, three months and 19 days, after the sale** — and never closes, because the published policy never closes.
...
- **Checkout Session, Payment Link, or invoice with `automatic_tax[enabled]=true`** — refunding the charge is enough for the reported tax to move. The Refunds page confirms the boundary: *"If you're using Stripe Tax APIs to record sales, you must [record refunds](https://docs.stripe.com/tax/payment-intent/custom#reversals)"* ([Refund and cancel payments](https://docs.stripe.com/refunds)) — the "if" is doing the work.

  **Checked absent, and it matters here.** Stripe's decrease list says a refund of a Checkout or invoice charge decreases the reported tax; it does **not** say how a **partial** refund of such a charge is apportioned in the reversal Stripe creates for you. Searched [Tax reporting](https://docs.stripe.com/tax/reports), [Refund and cancel payments](https://docs.stripe.com/refunds) and the Checkout tax guides: no statement of the apportionment rule for that path. The proportional split verified by the sandbox run below was on the **explicit `create_reversal`** path only. Since a partial refund is this product's *normal* case — most customers will have spent some Credits — this is a gap to close before Credits refunds depend on it: either verify a partial Checkout refund end to end and read the resulting reversal's `amount` and `amount_tax`, or take the sale through the Tax API path, where the split is documented and has been run.
