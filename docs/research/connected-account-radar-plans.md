...
2. **A second, always-on-the-platform fee exists that nothing in this codebase had accounted for.** Stripe bills a "Radar account fee" to the platform "always," independent of who pays per-transaction fees. Quantified today: $1.00 per active connected account/month on Standard, $3.00 on Plus, $5.00 on Pro. Under Doula Cloud's `fees.payer = account` configuration, the Practice pays the per-transaction fee, but Doula Cloud itself owes this second fee for every connected account on a paid tier, scaling with the number of active Practices rather than transaction volume. See §2.
...
**Checked absence, searched directly:** no field named for a Radar plan, tier, or `radar_options` appears anywhere in the v1 `Account` object, the v2 Core `Account` object, `Account Links` create, or `Account Sessions` create in the Stripe API reference. There is no way to set a connected account's Radar plan at account-creation time, or at any other time, through the API — the "Platform payments controls" Dashboard setting above is the only lever found.
...
**Stripe API reference**, searched via the Stripe MCP `stripe_api_search` / `stripe_api_details` tools for the v1 `Account` object, the v2 Core `Account` object, `Account Links` create, and `Account Sessions` create — no field naming a Radar plan, tier, or `radar_options` found on any of them.
