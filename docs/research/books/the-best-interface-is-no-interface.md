...
6. **A system reaching out proactively based on a person's inferred pattern, before being asked, is held up as the better model**, illustrated by a thermostat that stops asking and a hospital sensor that alerts hours ahead of a standard monitor (ch. 15, pp. 258, 260–261). Record: [#1266](https://github.com/markgoho/doula-cloud/issues/1266), which asked what a Practice with an untouched signup Credit grant hears after two years of silence, closed as not planned on 2026-09-10 — a ticket only, deciding nothing. What actually governs unattended silence here is ADR-0038's rule that every outbox is nudged by an act and a due date passing is not an act, and `api/internal/billing/dormancy.go`'s two-year notice, whose own comment ties the interval to New York's APL 1315(1-b) escheat deadline rather than to a proactive-reach-out judgment.
7. **A transparent, plain-language settings screen is worth more than a legal document, illustrated by an early Cortana privacy-settings screen** (ch. 17, pp. 273–274). Record: `docs/research/practicelite-onboarding.md`, a research document that names itself "inspiration only" and touches no ADR or implementation, records as a finding — not a decision — that Doula Cloud's signup form collects no consent of any kind, against a named competitor's separate Terms/Privacy and SMS-consent checkboxes.
