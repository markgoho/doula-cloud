...
| [Table](https://design-system.service.gov.uk/components/table/) | `organisms/DataTable.svelte` | Partly open -- walked 2026-08-30: aligned at a desktop width, but the six-column Staff table scrolls the whole page sideways at 320px ([#508](https://github.com/markgoho/doula-cloud/issues/508)). Right-alignment fixed -- [#509](https://github.com/markgoho/doula-cloud/issues/509): a `Column` takes an explicit `numeric?: boolean`, not inferred from the accessor's return type, since `accessor` already formats to a string (Billing's Quantity renders `"+3"`, not `3`) and there is nothing left to infer from at render time. `numeric: true` right-aligns the header with its body cells off one shared class, and also turns on `font-variant-numeric: tabular-nums`, settling the brief's separate "tabular figures wherever numbers are compared" rule for table columns at the same time -- right alignment without it is only half the job. Confirmed at 1280px and 320px on `/style-guide/data-table`'s Numeric column demo, 2026-08-30 |
