...
This is already how the product has been built. [ADR-0018](../adr/0018-templates-are-a-design-system-layer-with-two-named-exits.md)'s Template layer, the intake sequence drawn in `doula-cloud.pen`, and the three-box date of birth drawn in `b7e8ab7` all came from there. This document makes that explicit rather than leaving it as a habit that only holds while someone remembers.
...
## Where GOV.UK has no pattern at all

A row is not automatically Open just because GOV.UK is silent on it -- silence is a prompt to walk a fixed order of sources, not a licence to invent, per [ADR-0021's amendment](../adr/0021-govuk-is-the-reference-for-service-patterns.md#amendment-2026-09-01-what-to-do-when-govuk-has-no-pattern-at-all):

1. **GOV.UK Design System.**
2. **NHS.UK Design System and ONS Design System**, tied.
3. **Inclusive Components** (Heydon Pickering) and **Adrian Roselli's** writing, tied.
4. **Every Layout.**
5. **Original work, with a recorded reason.**

A row answered this way carries the status **Answered -- \<source\>** (e.g. `Answered -- ONS`), not Departed: nothing was overridden, because GOV.UK had no decision to override. See rule 5 below.
...
## Page titles: format, ownership, and the words a link uses

Page titles are Service Manual guidance, not a Design System component -- there is no catalogue row for `<title>`, which is why [#447](https://github.com/markgoho/doula-cloud/issues/447)'s axe scan caught the whole application having none of them ([#487](https://github.com/markgoho/doula-cloud/issues/487)) when this table's own walk could not have.

**Format.** `{Page} — {Service name}`, one em-dash, two segments -- GOV.UK's own `[Page] - [Service name] - GOV.UK` with the fixed third segment dropped, since nothing here plays that part. The Staff side's service name is `Doula Cloud`; the Client portal's is the Practice's own name, matching [#431](https://github.com/markgoho/doula-cloud/issues/431)'s Practice-named chrome -- so `formatPageTitle` takes an optional `serviceName` and every portal Template/route that knows its Practice passes it. A page showing a genuinely refused form gets GOV.UK's `Error: ` prefix, matching `ErrorSummary` ([#467](https://github.com/markgoho/doula-cloud/issues/467)); a data-load failure or a `+error.svelte` kind (404, 403, 503) does not, because its title already names the problem in words ("Page not found").

**Ownership.** One shared primitive, `#lib/components/PageTitle.svelte` (`formatPageTitle` in `#lib/pageTitle.ts`), not tiered under atoms/molecules/organisms/templates because it renders nothing visible -- it earns no `/style-guide` page. Every Template (`OverviewHub`, `RecordDetail`, `FormPage`, `QuestionPage`, `CheckAnswers`, `ErrorPage`) calls it internally with the same string handed to its `<h1>`/`<legend>`/`<label>`; every route with no Template -- archetypes A, C, F, G, the style guide, `/account` -- calls it directly. `pageTitle.usage.spec.ts` fails the pre-commit hook if a `+page.svelte` does neither, the same shape as `tokens.usage.spec.ts`. The Client portal's Practice name is preloaded through `portal/(authenticated)/engagements/[engagementId]/+layout.ts` rather than an onMount fetch, so the title (and the top bar) never flash from generic to Practice-named.

**Tracking.** The tab title says what the `<h1>` says, question by question on `QuestionPage` -- the title changes on every step of a journey, because that is 2.4.2's whole point. The style guide's ~70 demo pages get one static title (`Style guide — Doula Cloud`) from the shared `+layout.svelte`, not one per component.

**Link text should match the page it goes to -- with a carve-out GOV.UK itself draws.** A link whose sole visible text names a whole destination page (a nav item, a "Back to Clients" link) should read the same as that page's title once it is reached -- otherwise the destination surprises the person who followed it, and 2.4.4/2.4.9's own test (link purpose determinable from the link, or the link plus its immediate context) gets weaker with every mismatch. A spot survey while building #487 found this mostly already holds, and where it breaks down is instructive:

- **Nav items already comply.** `StaffTopBar`/`PortalTopBar`'s labels ("Clients", "Billing", "Your care") are the same strings now handed to those pages' `PageTitle`, because both were written from the same route inventory.
- **`DataTable`'s `rowHref` pattern already gets this right, structurally.** The linked cell's visible text is `column.accessor(row)` -- the row's own name, not a generic "View" -- so a Clients row already reads as "Jane Smith", which is what her `RecordDetail` page's own title will say. No separate disambiguating text is needed because the link text *is* the distinguishing content, for a sighted user and a screen-reader user navigating a links list alike.
- **`DataTable`'s `rowActions` column is a different category and this rule does not apply to it.** Staff's "Revoke"/"Print" buttons perform an action in place; they do not navigate to a page with its own title, so "link text matches destination title" has nothing to check there. (They may still want per-row disambiguating text for a *different* reason -- a screen-reader user hearing "Revoke, Revoke, Revoke" out of row context -- but that is a distinct accessible-naming concern from this one, not folded in here.)
- **Where it broke down, and is now fixed:** `practices/[practiceId]/clients/[clientId]/+page.svelte`'s bare `label="Edit"` link carried no distinguishing text -- the one place the spot survey found the rule genuinely violated. It now carries the same GOV.UK fix `CheckAnswers`'s Change links already used: a sibling visually-hidden span naming the Client, joined by `aria-describedby`, so the announced name becomes "Edit, Jane Smith". [#513](https://github.com/markgoho/doula-cloud/issues/513) walked every anchor in the application to confirm this was the one violation, not just a spot check.
- **CTA-style action links are a different case again, and this rule doesn't bite them.** "Manage staff", "Buy credits", "Payment settings", "Find or add a Client" and similar imperative links describe an action rather than purporting to name the destination page verbatim -- unlike a nav item or `DataTable`'s `rowHref`, whose whole visible text *is* a page or record's own name. A screen reader user still knows where each one goes from the phrase alone, so #513's audit did not treat a mismatch between "Manage staff" and the Staff page's own title as a violation. Same reasoning covers the Practice-name links (the switcher, the login/accept-invite/account pickers): the link text is the Practice's own name, the `OverviewHub` destination's `<h1>` is "Welcome to {name}" -- an amplification, not a surprise, the same shape as the `DataTable` row case above.
- **The GOV.UK Back link is a named exception, not a violation.** `BackLink`'s default label is literally "Back" -- GOV.UK's own pattern, reused by `QuestionPage` and `CheckAnswers` -- and #513's audit found two portal routes (`engagements/[engagementId]/contract`, `.../birth-plan`) hand-rolling the identical thing with the bare `Link` atom instead of the extracted molecule; both now use `BackLink`.
- **Repeated per-row action buttons are a distinct, separately-scoped problem.** `DataTable`'s `rowActions` and `DynamicFieldEditor`'s per-field "Move up"/"Move down"/"Remove" controls repeat the same bare word down a list -- the "Revoke, Revoke, Revoke" problem this table already flagged as out of #513's scope. #513 confirmed the category and filed the accessible-naming pass for it separately: [#515](https://github.com/markgoho/doula-cloud/issues/515).

#513 walked every anchor in the application -- nav, `DataTable` `rowHref`, the Practice pickers/switcher, all CTA links, the style guide's own component catalogue (exempt the same way `pageTitle.usage.spec.ts` already treats it: one static title covers every demo page), and every `Link`/`BackLink`/`ErrorPage` usage. The one violation above is fixed; nothing else in the walk needed a change.
...
| [Service unavailable](https://design-system.service.gov.uk/patterns/service-unavailable-pages/) | `templates/ErrorPage.svelte`'s `unavailable` state (503) | Aligned -- [#471](https://github.com/markgoho/doula-cloud/issues/471); walked 2026-08-30, once `/style-guide/error-page` grew the three states it was hiding |
...
| [There is a problem with the service](https://design-system.service.gov.uk/patterns/problem-with-the-service-pages/) | `templates/ErrorPage.svelte`'s `problem` state (500 and any other unexpected status) | Aligned -- [#471](https://github.com/markgoho/doula-cloud/issues/471); walked 2026-08-30, once `/style-guide/error-page` grew the three states it was hiding |
...
| [Error message](https://design-system.service.gov.uk/components/error-message/) | `LabeledField`'s `error` prop, passed by every form, above the control it refuses; `MembershipFields` renders a group's under its legend. Wording is gated: `formErrors.usage.spec.ts` fails a commit on "please", "valid", "invalid" or "required" in any component or route | Aligned -- built on [#467](https://github.com/markgoho/doula-cloud/issues/467); the message rendered *below* the control it refuses until `198953e` moved it between the hint and the control, found by walking the pages 2026-08-30 |
...
| [Summary list](https://design-system.service.gov.uk/components/summary-list/) | Two things, deliberately: `molecules/DescriptionList.svelte` for a record's facts, and `CheckAnswers`'s own rows where there is a Change action | Aligned -- [#464](https://github.com/markgoho/doula-cloud/issues/464) kept the action row internal to `CheckAnswers` rather than growing a molecule for one consumer; it moves out when a second page wants it; walked 2026-08-30, which found the two rendering GOV.UK's one component differently -- `CheckAnswers` drew a hairline under every row and `DescriptionList` drew none. Resolved -- [#511](https://github.com/markgoho/doula-cloud/issues/511): `DescriptionList` now draws the same hairline (`border-block-end` on `dt`/`dd`, GOV.UK's own grouping cue and the brief's Law of Uniform Connectedness -- "a run of rows sharing hairline dividers reads as one list"), so the two components render GOV.UK's one pattern the same way; confirmed at 1280px and 320px with a wrapped value on `/style-guide/description-list` |
| [Table](https://design-system.service.gov.uk/components/table/) | `organisms/DataTable.svelte` | Aligned -- walked 2026-08-30, resolved 2026-08-31. Right-alignment fixed -- [#509](https://github.com/markgoho/doula-cloud/issues/509): a `Column` takes an explicit `numeric?: boolean`, not inferred from the accessor's return type, since `accessor` already formats to a string (Billing's Quantity renders `"+3"`, not `3`) and there is nothing left to infer from at render time. `numeric: true` right-aligns the header with its body cells off one shared class, and also turns on `font-variant-numeric: tabular-nums`, settling the brief's separate "tabular figures wherever numbers are compared" rule for table columns at the same time -- right alignment without it is only half the job. Confirmed at 1280px and 320px on `/style-guide/data-table`'s Numeric column demo, 2026-08-30. Narrow-width overflow fixed -- [#508](https://github.com/markgoho/doula-cloud/issues/508): the six-column Staff Members table scrolled the whole page sideways at 320px because `DataTable` had only one markup tree. GOV.UK has no pattern here -- its own component page links out to a seven-year-old open discussion rather than an answer (ADR-0024) -- so per the source-precedence order ([#514](https://github.com/markgoho/doula-cloud/issues/514)) the narrow-width behaviour is **Answered -- ONS**: the ONS Design System's `responsive` table variant, not a departure, since there was no GOV.UK decision here to override. `DataTable` now renders a `<table>` when a `container-type: inline-size` wrapper can hold one and one `<dl>` per record (each column's label as `<dt>`, its value as `<dd>`) when it cannot, both generated from the same `columns`/`rows` props so no route hand-authors a second tree; the switch reads the component's own container, never the viewport. Confirmed at 320px on `/style-guide/data-table`'s six-column demo, which matches the Staff Members table's own column shape, and the continuum check (`continuum.svelte.spec.ts`) now asserts it on every change rather than once by hand, 2026-08-31 |
...
Five rules, so this does not become a document that was true once.
...
2. **A departure gets a row here with its reason** on the same commit that departs. A departure with no recorded reason is a defect, and it is the only kind of GOV.UK finding this project treats as one. **A departure is knowingly overriding a GOV.UK decision that exists.** When GOV.UK has no decision to override, rule 5 applies instead -- the row is not a departure and must not be recorded as one.
3. **A row that says Open names a ticket.** No row says "someday".
4. **A row only says Aligned once somebody has seen it render**, at a desktop width and at the floor set by [ADR-0024](../adr/0024-layout-is-intrinsic-and-320px-is-a-conformance-commitment.md) -- 320px, and stated there rather than here so the two cannot drift apart -- and it carries the date that happened. Source is not evidence here. This table's first version was written by reading the code, which is the right way to audit a *decision* -- whether a date is three inputs, whether a page asks one thing -- and the wrong way to audit a *rendering*. Four rows failed the first walk ([#475](https://github.com/markgoho/doula-cloud/issues/475), 2026-08-30) and none of them could have been caught any other way: the error message sat below its control, the six-column table scrolled the page sideways only at 320px, and two rows named a commit or a ticket that no longer said what the row claimed. `LabeledField`'s label did the same thing before them ([#425](https://github.com/markgoho/doula-cloud/issues/425)). A row whose walk date is older than the component it names has not been checked. **And a style-guide entry that shows one of a component's states hides the rest**: two of these rows could not be walked at all until `/style-guide/error-page` stopped demoing only `notFound`, and the six-column table defect was invisible because the Data table entry demos two columns.
5. **A row GOV.UK is silent on names which source answered it.** Walk [the source-precedence order](#where-govuk-has-no-pattern-at-all) and record the winning source as `Answered -- <source>`, e.g. `Answered -- ONS`. This is not a departure: rule 2's burden of a recorded reason is for overriding a GOV.UK decision, and there is none to override here.
