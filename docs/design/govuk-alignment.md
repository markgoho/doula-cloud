...
- **Where it breaks down:** `practices/[practiceId]/clients/[clientId]/+page.svelte` has a bare `label="Edit"` link with no distinguishing text at all -- the one place the survey found the rule genuinely violated. `CheckAnswers`'s own Change links already solved this the GOV.UK way (visually-hidden text after the word, via `Answer.changes`, `Link`'s existing `.visually-hidden` class), which is the pattern a fuller pass should reuse rather than invent again.

This survey was a spot check, not an audit -- it did not walk every anchor in the application. The full pass is [#513](https://github.com/markgoho/doula-cloud/issues/513), scoped to the concrete gap above rather than folded into #487.
