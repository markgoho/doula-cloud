...
- The two Scheduler jobs' `X-Internal-Secret` header value, **until [#1183](https://github.com/markgoho/doula-cloud/issues/1183) lands**. It was accepted here as unavoidable if the jobs are owned at all — it is a field on the job — and that reasoning held only while a shared string was the boundary. [ADR-0037](adr/0037-the-internal-boundary-is-a-caller-identity-not-a-shared-secret.md) replaces the header with an `oidc_token` block naming a service account, which is not a secret, so state stops carrying one at all rather than carrying one it has a good reason for. The code half is landed; the jobs move on #1183, against the runtime service account #1051 created.
...
Anyone with read on the state bucket therefore has the internal worker secret, for as long as the jobs still carry it. That is one principal and one service account, and it was the price of catching the next #481 — a price ADR-0037 stops paying rather than keeps justifying.
...
- **The Scheduler jobs still carry the old shared secret as a plaintext header value in the job spec**, readable by anything with `cloudscheduler.jobs.get`, which `roles/viewer` grants. The service already accepts the token that replaces it; moving the jobs is `terraform/scheduler.tf` plus the caller account in `terraform/iam.tf`, and the `INTERNAL_OIDC_*` variables in `terraform/cloud_run.tf`. → [Move the Scheduler jobs and the Cloud Tasks nudge onto ADR-0037's OIDC identity, and drop the shared secret from Cloud Run #1183](https://github.com/markgoho/doula-cloud/issues/1183)
