...
#797's "34 project-level IAM bindings" was the role count, read the same day as this page and before #1043 removed two of them. The number that matters for Terraform is the principal-role pairs — 37 then, 35 now — because `google_project_iam_member` is one resource per pair. Seventeen of those belong to Google's own service agents, which the boundary below leaves alone; the other eighteen are owned, down from twenty after #1043 removed `roles/cloudfunctions.developer` from `github-action-733741680@` and `roles/cloudfunctions.admin` from `firebase-adminsdk-rq3g0@`.
...
2. **`prevent_destroy` on everything that holds data or identity**, before the first apply, not after: the Cloud SQL instance, its databases, `doula-cloud-attachments`, every secret shell, and the Workload Identity provider. A `lifecycle { prevent_destroy = true }` block makes Terraform refuse the plan rather than execute it. Distinct from it, and easy to confuse with it: several providers carry their own `deletion_protection` **argument** — the Cloud Run v2 resource defaults it to `true`, and `google_sql_database_instance` mirrors the GCP-level flag. Set both deliberately. `prevent_destroy` is a Terraform-side refusal; `deletion_protection` is an attribute Terraform can also *turn off* in the same apply that then destroys the resource.
3. **Turn on Cloud SQL deletion protection.** `deletionProtectionEnabled` is `false` on the live instance right now — a GCP-level protection independent of anything Terraform believes. Setting it is one `gcloud` command and it should not wait for the Terraform work; it is #1044's first step.
...
**What depended on them: nothing.** Thirty days of Cloud Run request logs, re-read on 2026-09-08 immediately before deletion, against 10,000 entries rather than `gcloud logging read`'s 1,000-entry default — an earlier pass at the default limit undercounted, because `--order` defaults to newest-first and the newest 1,000 entries happened to land inside a single two-day window:
