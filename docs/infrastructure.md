...
| Service accounts | `github-action-733741680@`, `terraform-plan@` (#1044), `firebase-adminsdk-rq3g0@`, `firebase-app-hosting-compute@`, `850855848778-compute@` (Google-created default) | `github-action-733741680@` yes; the three Google-created ones no; `terraform-plan@`'s project roles are owned but the account itself is not yet a resource — see the by-hand table |
