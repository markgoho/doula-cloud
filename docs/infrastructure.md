...
- `roles/storage.objectUser` on `gs://doula-cloud-attachments`, held by `850855848778-compute@`. #1048 imported the bucket but not its IAM, so this grant was invisible to `plan`. The replacement grant on `doula-api-runtime@` is a resource in `terraform/storage.tf`. `terraform-plan@`'s own `roles/storage.legacyBucketReader` on this bucket is still unowned, which is a gap rather than a decision — filed separately.
