...
| `POST /api/staff/signup` | Bearer-token digest 5/hr, IP 50/hr | Once-per-person bootstrap event (`authn.BeginBootstrap`). Values are generous rather than tight — nothing has ever limited this endpoint before, so any finite cap is a real improvement — and sized above a single pass of the Playwright e2e suite's own call volume (one shared BFF and IP per run) and a 14-doula pilot agency's onboarding burst from one connection. A *repeated* e2e batch does exceed this from that one address; the harness clears its own counters when it does, and nothing about this sizing changed for it — see `docs/testing.md`, "Confirming a flake fixed". |
...
2. **Standard Status Code Usage**:
   ...
   * A signed-in caller whose identity resolves to no `staff` row gets `404 no matching staff account` on every pre-Practice `/api/staff/*` route, and `403` on a `/api/practices/{practiceId}/...` one (#1182): pre-Practice she asked about her own account and it does not exist, while behind a `{practiceId}` she asked about a Practice she has no standing at, which is the refusal beside it. `staffauth.requireSelf` owns the first, and its tests fail the build on a route in that family that answers anything else.
