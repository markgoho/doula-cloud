...
A row can also name a `reach`: an action `scan` runs, on the page it just `goto`'d, before its own two ready waits below ([#1240](https://github.com/markgoho/doula-cloud/issues/1240)). It exists because the route inventory is keyed by URL, and not every screen worth scanning has one of its own — some are a *state* behind a form submit rather than a place `goto` can land on directly. `reach` is how such a row says "act on the page, then scan" without turning into a script of its own: it drives only the acting-on-the-page part, and leaves the waiting for readiness to `scan`, exactly as every other row does. Most rows need none, since their `url` already is the screen. TOTP enrollment's second step — the QR code, the fallback secret and the code field, reached by answering step one's own password prompt — is the first row that does, and is scanned below.
