...
`@axe-core/playwright` scans forty-eight routes, covering every screen in the A–G layout-archetype table on [#405](https://github.com/markgoho/doula-cloud/issues/405) plus the ones that postdate it — the two settings screens #452's hub added, the Practice-deletion screen [#871](https://github.com/markgoho/doula-cloud/issues/871) built, the MFA-requirement screen enrolled here by [#894](https://github.com/markgoho/doula-cloud/issues/894), the two MFA-recovery screens [#694](https://github.com/markgoho/doula-cloud/issues/694) built, the portal's Messages page, the three under `clients/{clientId}` that [#516](https://github.com/markgoho/doula-cloud/issues/516) brought in (the Client detail hub, her edit screen, and the Engagement Request form), the contractor Doula's branch of `clients/search` that [#525](https://github.com/markgoho/doula-cloud/issues/525) added, the Practice-wide Invoice list from [#265](https://github.com/markgoho/doula-cloud/issues/265), the engagement-request approval screen that [#502](https://github.com/markgoho/doula-cloud/issues/502) built and [#528](https://github.com/markgoho/doula-cloud/issues/528) enrolled here, `/account`, given the same shell every other authenticated Staff route already has by [#484](https://github.com/markgoho/doula-cloud/issues/484), and the three archetype-A routes that need a live session — `/no-practice` ([#745](https://github.com/markgoho/doula-cloud/issues/745), enrolled here by [#749](https://github.com/markgoho/doula-cloud/issues/749)) and TOTP enrollment's own two steps at `/mfa/enroll` (step one: [#1114](https://github.com/markgoho/doula-cloud/issues/1114); step two: [#1240](https://github.com/markgoho/doula-cloud/issues/1240), see below) — a signed-in identity exchanged straight for a session with no `POST /api/staff/signup` in between, so the staff-session lookup `/no-practice` itself calls resolves to no Practice rather than to no session at all, which is what every other route in that batch gets scanned under. `mfa/enroll`'s own two rows read a stand-in answer off the same lookup instead, for the reason given below. All three are scanned under that one session, since none of them reads anything from it that a Practice would change. The route inventory is in the spec itself, typed and grouped by archetype; that table *is* the documented set, so adding a route means adding a row rather than remembering a convention. Five tests, one per session the routes need — signed out, no-Practice, Staff, Client portal, contractor Doula — because signup and login cost the same few seconds however many routes follow them, and the contractor branch renders under a session none of the other four can produce. A sixth and seventh test scan nothing: they are the two guards on the gate described below.
...
A row can also name a `reach`: an action `scan` runs, on the page it just `goto`'d, before its own three ready waits below ([#1240](https://github.com/markgoho/doula-cloud/issues/1240)). It exists because the route inventory is keyed by URL, and not every screen worth scanning has one of its own — some are a *state* behind a form submit rather than a place `goto` can land on directly. `reach` is how such a row says "act on the page, then scan" without turning into a script of its own: it drives the page to the target state and waits for whatever distinguishes *that* state, when the row's own `h1` cannot (TOTP enrollment's two steps render under one title, so a `reach` that only submitted step one's form and returned would leave `scan`'s `h1` wait satisfied by step one, not step two). `scan`'s own three ready waits still run after `reach` returns and still own whether the state `reach` reached is *finished loading* — a `reach` is never a substitute for them, only what gets the page to the state they should be waiting on. Most rows need no `reach` at all, since their `url` already is the screen. TOTP enrollment's second step — the QR code, the fallback secret and the code field, reached by answering step one's own password prompt and waiting for the QR code to render — is the first row that does, and is scanned below.

A `Template` (`FormPage`, `ListPage`, `OverviewHub`, `RecordDetail`) is always in one of three states: `loaded`, `loading`, or `loadError`, and a route in the inventory is meant to be scanned in exactly one — its `loaded` state, the actual screen. Every route waits on three things before axe runs, one per state it must NOT be in: its own `<h1>` visible (never *still on its way to the DOM*), the absence of any loading affordance on the page (never `loading`), and the absence of a `Template`'s own load-failure marker (never `loadError`). This matters more than it looks: the app is a client-rendered SPA, `goto` resolves long before the data lands, and axe against a half-painted or failed-to-load page finds a different set of violations every time — which CI's `retries: 2` would then quietly launder into green, the exact failure mode [ADR-0020](adr/0020-smoothness-is-gated-on-causes-because-the-outcome-is-not-measurable-where-the-gate-lives.md) documents.

The second wait is there because the first one was never a ready signal for most of the inventory ([#1152](https://github.com/markgoho/doula-cloud/issues/1152)). `FormPage` and `ListPage` both render their page title in their `loading` branch, beside a `Skeleton` standing where the content will be — deliberately, since those titles are static ("Your account", "Clients") rather than data a fetch has to bring, unlike `OverviewHub`'s and `RecordDetail`'s, which are suppressed while loading. So on every archetype C, E, F and G row, a visible `<h1>` could be satisfied with the placeholder still up. Measured on `settings/mfa` with its own impact read held open: axe found **zero violations against the Skeleton**. It exercised twenty-one rules there against the loaded screen's eighteen — the three extra (`aria-roles`, `aria-required-attr`, `aria-deprecated-role`) are the Skeleton's own `role="status"` being checked — while `<main>` held twenty-four nodes rather than twenty-nine, and one button rather than two, the missing one being the screen's own MFA-requirement toggle. The placeholder is not merely a thinner version of the screen: it is a different set of nodes, passing a different set of rules, and the control the route exists to offer is not among them. So the failure was not a false red or a flake — it was a clean scan of a placeholder, reported as a clean scan of the screen. The second wait is `expect(locator).toHaveCount(0)` against `[role="status"][aria-busy="true"]`, which is what `Skeleton` renders and what a screen-reader user hears; it retries rather than sleeping, and it fails loudly rather than falling through into a measurement, the same two properties [#1126](https://github.com/markgoho/doula-cloud/issues/1126)'s `awaitSettled` has on the unit side. It holds for every row at once, so a new row names nothing for it. That last property is why it was chosen over the two other shapes the ticket weighed. Naming a node per row — something only that route's loaded state renders — is a convention rather than a guarantee: it covers the row whose author remembered, and every row added later inherits the bug. Stopping `FormPage` from rendering its title while loading would change a screen a person sees in order to suit a test, and that title is a decision of its own ([#480](https://github.com/markgoho/doula-cloud/issues/480), and the prop's doc comment) — a page that shows its name while its content arrives beats one that shows nothing — so **`FormPage`'s loading-state title is unchanged by this**.

The guard on that is the sixth test, and it is one of two here that scan nothing: it holds `settings/mfa`'s impact read open, so the Skeleton never leaves, and asserts that the sweep **refuses** the route. A ready signal that stops being one fails silently — a green run cannot tell a scanned screen from a scanned placeholder — so the refusal is asserted directly rather than assumed.

The third wait exists for the same reason the second one does, one branch over ([#1258](https://github.com/markgoho/doula-cloud/issues/1258)): `FormPage` and `ListPage` render their `<h1>` and no `Skeleton` in their `loadError` branch too, beside a `Notice` and nothing else, so a route whose own data read fails under the e2e stack satisfied the first two waits already and axe scanned the one-`Notice` error screen while the row's key claimed it scanned the form or list. `[role="alert"]`, what `Notice`'s error variant renders, was tried first and refused on `accept-invite`: its no-token state ([#437](https://github.com/markgoho/doula-cloud/issues/437)) is `EntryPage`, not a `Template` with a `loadError` prop, and its "Missing invite token" `Notice` is that row's own, deliberately-scanned state — "the form is the page", per the comment ahead of the archetype-A batch above. That markup — an `<h1>` plus one `role="alert"` paragraph and nothing else — is byte-identical to a `Template`'s `loadError` branch, so no selector built from what `Notice` renders can tell the two apart; only the `Template` itself knows which one it is in. The third wait is `expect(locator).toHaveCount(0)` against `[data-load-error]`, an attribute `FormPage` and `ListPage` set on their own `loadError` branch and nothing else — presence-only, the same shape as Skeleton's own `aria-busy`, and invisible to sight and to assistive tech, so it changes nothing a person meets; the `Notice` inside still carries `role="alert"` on its own. It runs after the loading-affordance wait, not before: a route that went `loading` -> `loadError` would otherwise pass this wait during the `loading` window, before the attribute exists, then pass the loading wait once the Skeleton left — and axe would scan the error state regardless.

The guard on the third wait is the seventh test, the same shape as the sixth: it answers `settings/mfa`'s own impact read with a refusal rather than leaving it unanswered, so `loadError` is set instead of `loading` staying true forever, and asserts that the sweep **refuses** the route.

`OverviewHub` and `RecordDetail` carry no `data-load-error` of their own; they do not need it today, since both already suppress their `<h1>` while `loadError` (the same suppression that already protects them from the *loading* miss above), so the first wait already refuses them. That is a fact about their current markup, not a guarantee — a future static title on either would reopen exactly this gap.
...
Routes deliberately not scanned, so the gap is a decision rather than an oversight: `style-guide/*` (component demos, not archetypes — sixty pages for no additional archetype coverage), `demo/*` (SvelteKit scaffolding), and the two picker states of `/` — the Staff picker and the portal picker — which render the same nodes as the signed-out state of `/` that *is* scanned (an `EntryPage`, an `<h1>`, and a list of links) while costing a Staff member with two Memberships and a Portal Account with two Engagements to provision. The one node they add that the scanned state has not got is the portal picker's status paragraph, joined to its link by `aria-describedby`; that join is asserted in `Link.svelte.spec.ts` rather than here, since axe scans the DOM instances it is pointed at and not the pattern behind them. `/` itself scans as an archetype A route: [#357](https://github.com/markgoho/doula-cloud/issues/357) decided and built what `/` shows, and [#678](https://github.com/markgoho/doula-cloud/issues/678) decided that it carries a shell, moving it into the `(signed-out)` route group so it gets the reduced bar, the `<main>` landmark and the skip link that group already supplies. `/account` used to share the same shell-less gap, until [#484](https://github.com/markgoho/doula-cloud/issues/484) gave it the same shell every other authenticated Staff route already has; it now scans, as an archetype F route, in the spec's own route inventory above. `/mfa/enroll` was the last route left with that gap, and [#1114](https://github.com/markgoho/doula-cloud/issues/1114) closed it the way #678 closed `/`'s — by moving the route into the `(signed-out)` group, whose reduced bar is the true one for a person holding a session that has not cleared MFA and so has no Practice a Staff bar could name. Its step one, the password re-authentication, scans as an archetype A route behind the same Practice-less session `/no-practice` uses. **Its step two — the QR code, the fallback secret and the code field — now scans too ([#1240](https://github.com/markgoho/doula-cloud/issues/1240)),** using the `reach` shape described above: the row's `reach` answers step one's password prompt and waits for the QR code step two renders, so `scan`'s own three ready waits and axe both run against step two's screen rather than step one's. Step two was unreachable at all until [#1132](https://github.com/markgoho/doula-cloud/issues/1132): the vendored Firebase Auth emulator has no TOTP enrollment path (see the MFA section below), so the call that opens it 400s every time against the real emulator. #1132's `app/e2e/totpStub.ts` closes that at the browser's network boundary; the test installs it (`stubTotpFactor`) before scanning either step, and the row's `reach` drives step one's submit against it.

Reaching step two also had to close a race #1240 found rather than caused. `mfa/enroll`'s own `onMount` re-reads `/api/staff/session` for the email it re-authenticates with, and the no-Practice identity this session carries has no staff row at all -- that is the whole point of `seedAccountWithNoPractice` -- so the real endpoint answers 404 (`requireSelf`, `api/internal/staffauth/self.go`) and `onMount` sends the browser to `/login` the moment that resolves. Step one's own scan had never noticed: its `<h1>` paints at mount, before `onMount`'s async body has had a chance to run, so the retrying `toBeVisible` wait above always won that race by the time it mattered. Step two's `reach` does not get that head start -- filling the password field and clicking Continue is real interaction, long enough for the redirect to land first and leave nothing to click. It is answered at the network boundary instead of raced: the test's own `page.route` for `/api/staff/session` fulfills it with the account's own email, a 200 the same shape `stubTotpFactor` already answers identitytoolkit's calls with, installed once both `mfa/enroll` scans can use it. Confirmed to change nothing about what either scan measures beyond that one fetch: the `(signed-out)` layout (`app/src/routes/(signed-out)/+layout.svelte`) renders a fixed reduced bar with no session read of its own, so this is the only place either screen reads that response.
...
**A stack is orphaned when the age check and neither liveness signal below say otherwise.** Unlike a testcontainers Postgres, an e2e stack legitimately outlives any fixed budget — `bun run dev:full` shares it and runs for as long as somebody is working — so age alone must never authorize a reap, and a stack is live if *either* of the two signals below says so:

- **The worktree behind it has gone quiet.** Each worktree's `.port-offset` file says which project name it owns — the same scan `worktree-provision.ts` runs to refuse an offset another worktree holds — so an offset that no `.claude/worktrees/*` directory claims, or whose directory has not been touched in **30 minutes**, has nothing live behind it by this signal. Thirty minutes, read the same three ways (worktree directory, its git dir, that dir's index), is `worktree-prune.ts`'s own measure of "nobody is standing here", written because a session that has just landed its PR sits in a finished worktree for as long as it takes to write a summary. Only the quiet test is borrowed, not the bar: prune removes a directory only when it is quiet *and* clean *and* merged, because deleting a worktree can destroy unlanded work. Nothing here can — a reaped stack is a database the next `up -d` rebuilds, holding fixtures — which is why quiet plus the age check below is the whole bar. This reader fails closed: a worktree that is there but whose timestamps cannot be read counts as touched, and only a directory that is *gone* answers "nothing live" by this signal alone.
- **Its heartbeat file has gone stale.** The worktree-quiet check on its own misses a real case ([#1193](https://github.com/markgoho/doula-cloud/issues/1193)): a `bun run test:e2e` or `bun run dev:full` run in progress can go 30 minutes without touching the worktree directory, its git dir, or that dir's index at all — a long Playwright run, or a quiet stretch of interactive `dev:full`, outruns that check while the stack is very much in use. `app/e2e/stack.ts`'s `startStack`/`stopStack` keep a `.e2e-heartbeat` file beside the worktree's `.port-offset` fresh — touched once a minute — for exactly as long as the stack is actually being used, the same heartbeat shape `scripts/gate-lock.ts` already uses for its own lock. A stack whose heartbeat has been touched in the last **5 minutes** (five missed beats, comfortably past a slow tick) is live by this signal regardless of the worktree check. A killed session simply stops writing it, so this reader fails *open* rather than closed — a missing or unreadable heartbeat means only that this signal has nothing to add, and the worktree-quiet check above still applies on its own; an orphaned stack still ages into reapability once both readers agree nothing is live.
- **The stack itself is older than 15 minutes.** The weakest of the three, and there only to cover a stack brought up seconds ago inside a worktree whose files happen not to have been touched since — comfortably longer than `compose up -d` plus the migrate (≤90s) and `go build` (≤120s) steps that follow it.
...
### Confirming a flake fixed: repeating a batch, and the rate limiter in the way

The technique every flake on this tracker has been confirmed fixed by ([#827](https://github.com/markgoho/doula-cloud/issues/827), [#854](https://github.com/markgoho/doula-cloud/issues/854), [#958](https://github.com/markgoho/doula-cloud/issues/958)) is to run the affected specs many times over, concurrently, and see whether the timing bug reappears:

```sh
# Same shell exports as everything else here: DOCKER_HOST and
# TESTCONTAINERS_RYUK_DISABLED=true.
cd app
bunx playwright test staff-invite-role mail-delivery mfa-required --repeat-each=10 --workers=3
```

The BFF rate-limits its pre-account routes per caller address (`api/internal/ratelimit`, sized by [`api-design.md`](api-design.md) section 6), and **the whole suite is one caller address**. `POST /api/staff/signup` allows that address 50 requests an hour and `POST /api/session` allows it 100 — so a repeated batch spends a budget a person never would. The command above seeds six founding Owners and about nine sign-ins per repeat, which used to mean eight clean repeats and then `429 RATE_LIMITED` on everything after, with a `Retry-After` of nearly an hour the run could not outlast ([#1138](https://github.com/markgoho/doula-cloud/issues/1138)).

The escape hatch is `retryPastRateLimit` (`app/e2e/rateLimit.ts`), which the three fixtures that reach a rate-limited route already send through: `seedFoundingOwner` (signup), `auth.ts`'s `signIn` (`POST /api/session`, which `signInEnrolled` also goes through), and `acceptStaffInvite` (`POST /api/staff/accept-invite`, fronted by the same `bootstrapRules` as signup). It is **reactive**: it sends the request, and only if a 429 actually comes back does it call `resetRateLimits` (`app/e2e/stack.ts`) and send once more. Nothing is pre-cleared, no budget is raised, and a single-pass run and CI behave exactly as they did.

`resetRateLimits` is a `DELETE FROM rate_limit_buckets` issued by psql inside the e2e stack's own compose database — the same `execSQL` seam `seedEngagement` and `seedPracticeRate` write rows through. There is no environment variable, header or product code path involved, so there is nothing here that could be turned on against a deployed BFF: the mechanism *is* holding the database, and migration `00060` grants `app_runtime` only `SELECT`/`INSERT`/`UPDATE` on that table, so the role the BFF connects as could not run the statement at all. The limiter is untouched and still refuses a genuine burst — `TestSignupRefusesAGenuineBurst` (`api/internal/staffauth`) drives the mounted signup route 51 times and asserts the 429, so this escape hatch cannot quietly become a disabled limiter.

If you add a fixture that posts to another rate-limited route and a repeated batch starts failing on it, send that call through `retryPastRateLimit` too rather than raising anything on the BFF side. A batch is currently proven clean at `--repeat-each=30 --workers=3` across those three specs plus `portal-invite-accept` (180 tests, and roughly 180 signups, 270 sign-ins and 120 invitation acceptances — every one of those three budgets spent several times over).

[#1254](https://github.com/markgoho/doula-cloud/issues/1254) threaded an `APIRequestContext` through `stack.ts`'s `readStaffInviteToken`/`readInviteTokenFromMailbox` so the mailbox fallback spends `outboxMail.ts`'s shared `readMailbox` instead of a second hand-rolled `fetch`. That fallback only reads the mailbox after its own SQL check finds the `staff_invite_outbox` row already terminal, and `outbox.MailWorker` always sends before it marks a row terminal inside the same transaction — so, unlike the race [#1141](https://github.com/markgoho/doula-cloud/issues/1141) fixed, there is nothing here for a concurrent drain to race against. Confirmed with `bunx playwright test staff-invite-role mfa-required mfa-recovery mail-delivery simulation-world --repeat-each=10 --workers=3` (90 tests, no failures) — `mail-delivery.e2e.ts` is the concurrent drainer that supplies the contention rather than a caller of `readStaffInviteToken` itself.
...
There is no local fix for a *real* TOTP code: deriving a valid one from a known secret needs a TOTP-capable server, and the local emulator is not one. That recipe works against the *deployed* app's real Identity Platform project instead — enroll by hand once through <https://doula-cloud-app.web.app> and keep the secret for future runs. What a Playwright spec can do locally, since [#1132](https://github.com/markgoho/doula-cloud/issues/1132), is drive the TOTP screens themselves: `app/e2e/totpStub.ts` relabels the emulator's own second factor at the browser's network boundary, so `/login`'s code challenge, `/mfa/enroll` and the MFA-recovery step-up are all walked for real, with only the factor's provider string and the correctness of the six digits faked. Its header comment is the record of what that costs. What the emulator *does* support is PHONE_SMS enrollment, and the second-factor gate in `staffauth.Middleware` only checks the `firebase.sign_in_second_factor` claim — it does not care which provider produced it. `app/e2e/mfa.ts` already enrolls a phone factor as a stand-in and has since #606; that is the pattern this reuses rather than a new bypass.
