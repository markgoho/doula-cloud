...
A row can also name a `reach`: an action `scan` runs, on the page it just `goto`'d, before its own two ready waits below ([#1240](https://github.com/markgoho/doula-cloud/issues/1240)). It exists because the route inventory is keyed by URL, and not every screen worth scanning has one of its own — some are a *state* behind a form submit rather than a place `goto` can land on directly. `reach` is how such a row says "act on the page, then scan" without turning into a script of its own: it drives the page to the target state and waits for whatever distinguishes *that* state, when the row's own `h1` cannot (TOTP enrollment's two steps render under one title, so a `reach` that only submitted step one's form and returned would leave `scan`'s `h1` wait satisfied by step one, not step two). `scan`'s own two ready waits still run after `reach` returns and still own whether the state `reach` reached is *finished loading* — a `reach` is never a substitute for them, only what gets the page to the state they should be waiting on. Most rows need no `reach` at all, since their `url` already is the screen. TOTP enrollment's second step — the QR code, the fallback secret and the code field, reached by answering step one's own password prompt and waiting for the QR code to render — is the first row that does, and is scanned below.

Every route waits on two things before axe runs: its own `<h1>`, and the absence of any loading affordance on the page. This matters more than it looks: the app is a client-rendered SPA, `goto` resolves long before the data lands, and axe against a half-painted page finds a different set of violations every time — which CI's `retries: 2` would then quietly launder into green, the exact failure mode [ADR-0020](adr/0020-smoothness-is-gated-on-causes-because-the-outcome-is-not-measurable-where-the-gate-lives.md) documents.
