...
**Always set `GOLANGCI_LINT_CACHE` to a path under `--show-toplevel`, never bare
`golangci-lint run`.** Without it, golangci-lint's results cache defaults to one shared
location (`~/.cache/golangci-lint` / `~/Library/Caches/golangci-lint`) across every worktree
on the machine, keyed in a way that does not account for a worktree's path being reused or
removed. A session linting after another worktree was pruned can then see findings that
point at files under a worktree that no longer exists on disk, or -- more dangerous -- a
stale "clean" entry can mask a real issue in the current worktree's own changed file.
`--show-toplevel` resolves to the current worktree's own root (see
`docs/agents/worktree-flow.md`), so the cache lives and dies with that worktree and never
leaks into another one; `.golangci-cache` is gitignored. If `golangci-lint run` ever reports
issues in files that don't exist in your working tree, that's this problem -- run
`golangci-lint cache clean` (with the same `GOLANGCI_LINT_CACHE` set) and rerun.
