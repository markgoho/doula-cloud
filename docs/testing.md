...
Routes deliberately not scanned, so the gap is a decision rather than an oversight: `style-guide/*` (component demos, not archetypes — sixty pages for no additional archetype coverage), `demo/*` (SvelteKit scaffolding), and `/`, which renders with no shell, no `<main>` and no skip link at all — [#357](https://github.com/markgoho/doula-cloud/issues/357) decided and built what `/` shows, but never whether it should carry a shell, which is tracked separately as [#678](https://github.com/markgoho/doula-cloud/issues/678). `/account` used to share this same gap, until [#484](https://github.com/markgoho/doula-cloud/issues/484) gave it the same shell every other authenticated Staff route already has; it now scans, as an archetype F route, in the spec's own route inventory above.
