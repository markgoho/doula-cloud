...
| 6.2-a | Keep a copy of what she signed | **Download signed Contract (PDF)** is offered on her voided Contract, and the file that downloads is what she signed. The control is gated on the Contract read's own `hasSignedPdf`, which the Go BFF fills from the same lookup the signed-PDF route streams from ([#1119](https://github.com/markgoho/doula-cloud/issues/1119) closed) — so the screen asks the endpoint's own question rather than a second reading of `status` that drifted away from it. A Contract that was never signed still offers nothing, and the endpoint still 404s for it ([HS-G3](https://github.com/markgoho/doula-cloud/issues/302), [NH-G8](https://github.com/markgoho/doula-cloud/issues/299) closed) | `manual` |
