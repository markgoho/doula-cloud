...
**What the pass declined to re-mark is the more useful half.** Both new specs run as the founding Owner, so a step whose whole claim is *what an Admin gets*, or *what a contractor gets*, is not driven by them however closely the clicks match — the [four marks](#the-four-marks)' own rule is that a spec counts only when it exercises the step the way the Persona would. That is why Dee Whitlock's Contract cells, Lena Vasquez's Add Client cell and Renata Alvarez's Visits cells are all corrected in place and all still `manual`. Two things it found are filed rather than left as prose: [#1119](https://github.com/markgoho/doula-cloud/issues/1119), where a voided Contract's signed PDF was served by the API and hidden by both screens (fixed, and Nadia Haddad's 6.2-a re-marked `manual` with it — the first row of the table above to move since), and [#1121](https://github.com/markgoho/doula-cloud/issues/1121), which holds the same stale Add Client story on Tasha Bell's and Maya Okonkwo's plans — outside #685's six, and not half-correctable without a re-mark of its own.

**[#1121](https://github.com/markgoho/doula-cloud/issues/1121) closed that remainder**, as a desk pass in the same shape: evaluator-doula.md's 5.1, 5.2, 5.2-a and 7.1-a and solo-birth-doula.md's 3.1, 3.1-a, 3.2, 3.2-a and 3.4 were read against the code and corrected, each plan carrying a dated Run-log section naming every cell it moved. Both plans gained a step — Tasha's 5.2-b and Maya's 3.2-b — because ADR-0017 split one act into two: the save of a Client is free, and the Engagement Request that follows it is what creates the Engagement and locks the Credit. Three marks moved: Tasha's 5.1 and Maya's 3.1 become `automated (add-client-visits.e2e.ts)`, since that spec runs as the founding Owner both Personas are and opens and answers intake's first question the way each would; Maya's 3.1-a leaves `missing-feature (MO-G3)` for `manual` because [#252](https://github.com/markgoho/doula-cloud/issues/252) is closed and the step can be performed. **What it declined to re-mark is again the useful half**: the save cells themselves — Tasha's 5.2, Maya's 3.2 — stay `manual`, because what they now claim is that *no* Engagement and *no* ledger row appear, and `add-client-visits.e2e.ts` asserts only where the save lands. A spec that does not assert the step's own result does not cover it, however closely the clicks match. The same pass corrected the mechanical Add Client claims on three journey maps — Tasha's, Maya's and Dee Whitlock's — which had kept the pre-ADR-0017 story after their plans were corrected, and Dee's own plan named a button, **Add Client**, that no longer exists. Two live gap rows still tell the old story and are held at [#1236](https://github.com/markgoho/doula-cloud/issues/1236), because correcting either means re-reading the gap issue that owns it rather than reading prose. Reading the Engagement Request form for Maya's 3.4 also found a defect and filed it: an approver on an empty balance is promised `Balance after -1` ([#1235](https://github.com/markgoho/doula-cloud/issues/1235)). The run-status table above is recounted from both Steps tables again, and the Total recomputed from its own rows to 41 / 149 / 0 / 52, which includes [#1119](https://github.com/markgoho/doula-cloud/issues/1119)'s row — that landed on trunk while this pass was in flight, and both passes moved the same line.
...
**No check would have caught this drift.** Nothing in `app/` or CI reads these documents: a Marks summary can disagree with its own Steps table, and the Total can disagree with every row, and the build stays green — which is how three summaries were found wrong in #685 and two more corrected here. [#1233](https://github.com/markgoho/doula-cloud/issues/1233) holds the arithmetic half of that, which is the half a machine can check, and it landed as `app/src/lib/testPlans.usage.spec.ts`: it recomputes every plan's Marks summary from its own Steps table, this README's run-status table and Total from the plans, every mark against the four this section defines, every `automated (<spec>)` against `app/e2e/`, and every `missing-feature (<gap id>)` against the gap tables below. It found one live disagreement on landing — employed-doula.md's Stage 5 table carried a stray blank line that orphaned step 5.2-a from the table GitHub renders it as part of, so a recount of the Steps table read one `manual` step short of the Marks summary — and the blank line was removed in the same change.

**[#873](https://github.com/markgoho/doula-cloud/issues/873) corrected six findings the 2026-09-06/07 triage of #319's children had falsified** on evaluator-doula.md's map and plan: five gaps — TB-G2, TB-G3, TB-G4, TB-G5 and TB-G7 — had each moved, closed or narrowed since the findings were written, and a sixth claim (a Credit covers one Engagement, not one Client) was already correct and needed no change. The documents still stated the five old gap versions. A dated Run-log section on the plan names what moved. One mark changed: 6.1 leaves `missing-feature (TB-G5)` for `manual`, because the export and the account deletion it named are both built. The run-status table above is recounted from evaluator-doula.md's Steps table, Total with it, to 40 / 153 / 0 / 49.

**[#1241](https://github.com/markgoho/doula-cloud/issues/1241) closed the four stages [#1135](https://github.com/markgoho/doula-cloud/issues/1135) left held on returning-postpartum-client.md**: stages 1, 4, 7 and 8 each named a gap that had since closed — MO-G4 (#253), CB-G2 (#308), CB-G5 (#311) and CB-G6 (#312) — and both the map and the plan still told the old story. A dated Run-log section on the plan names what moved. Five marks changed, all `missing-feature` to `manual`: 1.1 because `TransitionHandler` now completes an Engagement; 4.1 because the Engagement Request that creates a second Engagement asks birth-or-postpartum and the Engagement carries `kind` from creation; 4.1-a because the status-to-`postpartum` approximation is both refused (the status value no longer exists) and unneeded now that 4.1 records the fact directly; 7.1/7.2/7.2-a because the Birth Plan nav item is left out entirely where it does not apply and the route itself renders the portal's ordinary not-found page rather than a promise; and 8.1-a because the Client detail hub's Engagements table and merged History (#494) answer "have I cared for this person before" for Priya, and the portal root list answers it for Camille. That leaves her with no `missing-feature` step at all. The run-status table above is recounted from her Steps table, Total with it, to 40 / 158 / 0 / 44. Stage 3's CB-G1 row still tells the pre-ADR-0017 story and is held at [#1236](https://github.com/markgoho/doula-cloud/issues/1236).
