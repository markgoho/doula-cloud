...
Every entry has an **Observed** block. Some entries also have a **Narrated** block. They are two labeled blocks in the format, so telling them apart is never a reader's judgment call.
...
**An email is an artifact, and what it says is admissible.** A persona opens her mail in a browser at the sandbox mailbox ([environment.md](environment.md#how-a-persona-receives-her-email)) and follows the link out of the message, so the message page screenshots like any other screen and anchors a claim the same way. A wrong subject line, a link that goes nowhere, a body addressing her as `[object Object]`, a Reply-To that is `noreply` where ADR-0011 says it should be monitored — each is an ordinary entry with an anchor. What is *not* an artifact is anything about the message's journey: see the next section.
